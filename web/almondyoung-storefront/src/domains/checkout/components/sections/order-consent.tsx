...
import { PolicyDocument, type PolicyDocKey } from "@/domains/checkout/policies"
...
export function OrderConsentSection() {
  ...
  const docs = [
    {
      key: "terms",
      label: t("docs.purchaseTerms"),
      title: t("docTitles.terms"),
    },
    {
      key: "privacy",
      label: t("docs.personalInfo"),
      title: t("docTitles.privacy"),
    },
        ...
        {docs.map((doc) => (
          <Dialog key={doc.key}>
            <DialogTrigger className="flex w-full cursor-pointer items-center justify-between gap-3 py-2 text-left">
              ...
              <span className="shrink-0 text-xs text-gray-500 underline underline-offset-2">
                {t("view")}
              </span>
            </DialogTrigger>
            <DialogContent className="flex h-[80vh] max-w-[calc(100%-2rem)] flex-col gap-0 p-0 sm:max-w-3xl">
              ...
              <div className="min-h-0 flex-1 overflow-y-auto px-5 py-5">
                <PolicyDocument docKey={doc.key as PolicyDocKey} />
              </div>
            </DialogContent>
          </Dialog>
        ))}

        <p className="mt-3 text-center text-[12px] leading-relaxed text-gray-500 lg:hidden">
          {t("agreementNotice")}
        </p>
      </div>
    </section>
