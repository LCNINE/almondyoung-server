...
import { getParentAccessToken, hasParentRefreshToken } from "./parent-cookies";
...
/**
 * parent cookie 의 access/refresh 토큰을 user-service 에 검증 위임해 활성 세션 userId 를 얻는다.
 ...
 * cookie 갱신이 필요하면 호출자(server action)가 명시적으로 setParentAuthCookies 를 수행한다.
 */
export async function getActiveSessionUserId(): Promise<string | null> {
  ...
  const refreshToken = await hasParentRefreshToken();
