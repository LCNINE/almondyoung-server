...
작성 2026-06-29. **Phase 0·1·A(클러스터 A) 구현·develop 머지·live 마이그레이션 적용·배포 완료(2026-07-01 — core `/health`·`inventory/health/ready` GREEN, `nest build core` 클린). 다음 = Cluster B. live FO 10건의 SO:FO unique 게이트는 read-only 점검 결과 클린(중복·SO-merge 흔적 0, 후속 추적 #5 해소) → B = 1 PR.** 이 문서가 본 프로젝트의 상황판(허브)이다.
...
| Phase 3 FO 정체성 (클러스터 B) | 🟦 설계 확정·게이트 클린 | 구조만(제거+개방+강제), 흐름 미룸. FOL drop·split/merge 은퇴·M:N 개방·SO:FO unique. 1 PR. **B 선결 게이트(후속 추적 #5) read-only 점검 = 위반 0** → 클린 1 PR 확정. 상세 = Resolved Decisions §Phase 3 (클러스터 B). |
...
9. ⏭️ **다음 = Cluster B 구현** (구조만: FOL drop · FO-split/SO-merge/dead 중복출고경로 제거 · FO↔상자 M:N 개방(`uqFulfillmentOrder` drop + `openedForFO` nullable) · `fulfillmentOrders(salesOrderId) WHERE NOT NULL` 부분 unique). 상세 = Resolved Decisions §Phase 3 (클러스터 B). A→B 순서. **✅ 게이트 해소(후속 추적 #5): live FO 10건 read-only 점검 = 위반 0(중복 SO 0·SO-merge 흔적 0·전부 1:1, FOL 0)** → 부분 unique 클린 추가 가능, B = 1 PR(데이터 정리 불필요). caveat: 배포 직전 출고가 프로덕션 라이브가 됐으면 점검 재실행.
