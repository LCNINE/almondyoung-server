...
| Phase 2 상자 워크플로 (클러스터 A) | 🟩 구현·develop 미머지(브랜치 `feature/outbound-ahipment-ledger`, 통합 미검증) | 검수 영속 붕괴(`inspection_sessions`/`items`→`shipment`/`shipment_line`)·`shipment_line{inspectedQty,forced}`·invoice FK 뒤집기(`invoice.shipmentId`)·자동완료 소진(`inspectScan`→`consumeShipment`)·박스 lazy 생성(`openBoxByScan`)·명시적 ship(`POST :id/ship`) 은퇴. **단위 GREEN**(fulfillments/shipment/outbound-consumption/invoice·hanjin/direct-ship 135)·`nest build core` 클린. 통합 spec(`outbound-consumption.integration`)은 새 박스/검수 모델로 갱신했으나 `describeIfDb` skip(빚 — DATABASE_URL 환경에서 실증 필요). 상세 = Resolved Decisions §Phase 2 (클러스터 A). |
...
8. ~~**Cluster A 구현** (TDD, 1 PR): 검수 영속 붕괴(`inspection_sessions`/`items`→`shipment`/`shipment_line`, `inspection_issues` 유지)·`shipment_line{qty,inspectedQty,forced}`·검수 write-ownership 역전(`FOI.shippedQty` consume-driven)·`invoice.shipmentId` FK 뒤집기·자동완료 소진 트리거·박스 lazy 생성·명시적 ship 엔드포인트 은퇴~~ ✅ **구현 완료**(브랜치 `feature/outbound-ahipment-ledger`, develop 미머지). 단위 GREEN(135)·`nest build core` 클린·통합 spec 새 모델 갱신(skip 유지).
9. ⏭️ **다음 = Cluster B 구현** (구조만: FOL drop · FO-split/SO-merge/dead 중복출고경로 제거 · FO↔상자 M:N 개방(`uqFulfillmentOrder` drop + `openedForFO` nullable) · `fulfillmentOrders(salesOrderId) WHERE NOT NULL` 부분 unique). 상세 = Resolved Decisions §Phase 3 (클러스터 B). A→B 순서.
...
3. **마이그레이션 정리 nit**: `idx_shipments_opened_for_fo`(신규 평인덱스)가 `uq_shipments_fulfillment_order_id` UNIQUE(동일 단일 컬럼)와 중복 — 둘 다 `openedForFulfillmentOrderId` 단일 컬럼이라 평인덱스가 잉여(무해, 쓰기 비용만). **사용자가 migration 적용 전 리뷰 시 제거 고려** (재생성·적용은 본 작업 범위 밖).
