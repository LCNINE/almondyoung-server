...
| Phase 0 고리 닫기 | ✅ 완료(통합검증 실증 2026-07-02) | FIFO 순수함수 단위(5/5) + 재배선 단위 GREEN, 빌드 클린. **develop 머지됨**(Phase 1 과 함께 squash 커밋 `d67e89f6f`). 통합 스펙 실증 완료(2026-07-02, throwaway 로컬 Postgres — on_hand N↓·available 불변·SHIP 1건). |
| Phase 1 상자 라인 | ✅ 완료(통합검증 실증 2026-07-02) | 스키마(`shipment_lines`+`shipments.openedBy`)·`consumeShipment`/`ensureShipmentLines`·drop_ship 가드·fail-loud·journal(openedBy 귀속)·operator 배선 완료. 단위 GREEN(fulfillments 85, outbound-consumption 5, +invoice/direct-ship 회귀)·`nest build core` 클린. **develop 머지됨**(squash `d67e89f6f`; 머지 중 catalog 충돌 2건+중복 import 해소). **마이그레이션 `20260629092035` 적용 완료.** 통합 스펙(FIFO SQL·available 불변·작업자 journal) 실증 완료(2026-07-02, throwaway 로컬 Postgres — `describeIfDb` 는 DATABASE_URL 게이트로 열려 실행). |
| Phase 2 상자 워크플로 (클러스터 A) | 🟩 develop 머지·live 마이그레이션 적용·배포 | 검수 영속 붕괴(`inspection_sessions`/`items`→`shipment`/`shipment_line`)·`shipment_line{inspectedQty,forced}`·invoice FK 뒤집기(`invoice.shipmentId`)·자동완료 소진(`inspectScan`→`consumeShipment`)·박스 lazy 생성(`openBoxByScan`)·명시적 ship(`POST :id/ship`) 은퇴. **단위 GREEN**(fulfillments/shipment/outbound-consumption/invoice·hanjin/direct-ship 135)·`nest build core` 클린. 통합 spec(`outbound-consumption.integration`) 새 박스/검수 모델로 갱신 → **실증 완료(2026-07-02): `openBoxByScan→inspectScan` 자동완료 e2e 포함 5개 GREEN**. **develop squash-머지(`b5fa53357`) + 마이그레이션 ordering 수정(`b3cbb0113`); 마이그레이션 2개(`20260630125603`+`20260630155417`) live 적용·배포(core 헬스/ready GREEN).** live FO 10건/FOI 28건 발견 — B 게이트 read-only 점검 클린(위반 0, 후속 추적 #5 해소). 상세 = Resolved Decisions §Phase 2 (클러스터 A). |
| Phase 3 FO 정체성 (클러스터 B) | 🟩 구현·리뷰 완료(미머지) | 구조만(제거+개방+강제). FOL drop·FO-split/SO-merge/dead 중복출고경로 제거·shipments FO-unique drop(M:N 개방)·SO:FO 부분 unique. 브랜치 `feature/outbound-cluster-b-fo-identity`, 5 커밋(스키마+마이그레이션 / FO-split / SO-merge / dead 출고경로 / 검증·문서). subagent-driven(태스크별 spec+코드 2단계 리뷰; SO-merge 에서 ctor positional 회귀 1건 리뷰가 포착·수정). `nest build core` 클린·fulfillment spec 77 GREEN·sales-order spec base 패리티(사전 red 16). 마이그레이션 live 드라이런 4문장 클린. 인접 dead(Task 6)·admin-web(split-tab 404) 후속. 상세 = Resolved Decisions §Phase 3 (클러스터 B). |
...
3. ~~⚠️ **Phase 0 deploy 선결조건**: develop 빌드가 `#472`(overseas customs)의 catalog 컴파일 에러 2건으로 깨져 있음~~ ✅ **해소** — 머지(`d67e89f6f`) 시 catalog 충돌 2건(`product-versions.service.ts` import = `type ProductSnapshot`, `projection-snapshot.assembler.ts` = `isOverseas ?? false`)+auto-merge 중복 import 해소, `nest build core` 클린.
4. ~~Phase 1(상자 라인 + packing 연산) 스키마·연산 상세 설계~~ ✅ (Resolved Decisions §Phase 1 상세 설계).
5. ~~**Phase 1 구현** (TDD): 스키마(`shipment_lines`+`shipments.openedBy`)·`consumeShipment`/`ensureShipmentLines`·drop_ship 가드·fail-loud·`ship()` journalId·operator→openedBy 배선·`ship()`→`consumeShipment` 전환~~ ✅ **완료·develop 머지(`d67e89f6f`)·마이그레이션 `20260629092035` 적용 완료.** 단위 GREEN + 빌드 클린.
6. ✅ **통합 검증 빚 청산(2026-07-02)**: `outbound-consumption.integration.spec.ts` 를 **throwaway 로컬 Postgres**(dev 삭제·live 금지 회피)에 core 마이그레이션 31개 적용 후 실행 — 성공 기준 5개 GREEN(on_hand N↓·available 불변·SHIP 1건/라인·작업자 journal 귀속 + `openBoxByScan→inspectScan` 자동완료 e2e). **프로덕션 버그 0건**; 단위 mock 이 못 잡은 건 없었고, spec 하네스 결함 2건만 수정 — (a) 가짜 `DbService` 대역에 ADR-0025 단일 러너 `run(fn,tx)` 누락(spec 이 skip 이라 방치), (b) `stockSummary` 는 view 라 `wmsTables` 가 아닌 `wmsViews` 네임스페이스(`wmsTables.stockSummary`→`wmsViews.stockSummary`). 실행 레시피는 spec 헤더 주석에 기록.
7. ~~Phase 2 — FO=스냅샷 (destructive)~~ → **재설계·분해 확정·머지** (grilled 2026-06-30, 설계 커밋 `53a55a7aa`). 원 단일 "Phase 2"가 사실 독립 restructure 2덩어리라 분해: **Phase 2 = 클러스터 A(상자 워크플로)**, **Phase 3 = 클러스터 B(FO 정체성)**. 프로덕션 FO 0건이라 expand-contract 세리머니 붕괴 → **각 1 PR, A→B 순서.** 상세 = Resolved Decisions §Phase 2 (클러스터 A)·§Phase 3 (클러스터 B). ADR-0027 #1/#4 종결 트리거 amend(검수 자동완료).
8. ~~**Cluster A 구현** (TDD, 1 PR): 검수 영속 붕괴(`inspection_sessions`/`items`→`shipment`/`shipment_line`, `inspection_issues` 유지)·`shipment_line{qty,inspectedQty,forced}`·검수 write-ownership 역전(`FOI.shippedQty` consume-driven)·`invoice.shipmentId` FK 뒤집기·자동완료 소진 트리거·박스 lazy 생성·명시적 ship 엔드포인트 은퇴~~ ✅ **develop squash-머지(`b5fa53357`)·마이그레이션 2개 live 적용·배포 완료.** 단위 GREEN(135)·`nest build core` 클린·통합 spec 새 모델 갱신(skip 유지). 적용 시 drizzle DDL ordering 버그 2건 발견·수정(`b3cbb0113` — 후속 추적 #6).
9. ✅ **Cluster B 구현 완료(브랜치 `feature/outbound-cluster-b-fo-identity`, 미머지)** — FOL drop · FO-split/SO-merge/dead 중복출고경로 제거 · shipments FO-unique(`uq_shipments_fo_active`) drop(M:N 개방; `openedForFO` 는 A 가 이미 nullable 로 사전 스테이징) · `fulfillment_orders(sales_order_id) WHERE NOT NULL` 부분 unique(`uq_fulfillment_orders_sales_order`). 계획 = `docs/superpowers/plans/2026-07-01-cluster-b-fo-identity.md`. subagent-driven 실행(태스크별 spec+코드 2단계 리뷰 — SO-merge 에서 ctor positional 회귀 1건을 리뷰가 포착·수정). 마이그레이션 `20260630225428` live BEGIN…ROLLBACK 드라이런 4문장 클린(부분 unique 실데이터 충돌 없이 생성 실증). **다음: PR·머지·배포 → 배포 후 read-only 게이트 재점검**(caveat: 배포 직전 출고가 프로덕션 라이브가 됐으면 재점검 필수). 후속(별도): 인접 dead 정리(createFulfillmentOrder 등, Task 6 defer)·admin-web split-tab/merge 클라이언트 정리(후속 추적 #1).
...
4. ✅ **해소 (2026-07-02) — 통합검증 빚**: `outbound-consumption.integration.spec.ts` 를 throwaway 로컬 Postgres(dev 삭제·live 금지 회피)에 마이그레이션 적용 후 실행 → "on_hand N↓·available 불변·SHIP 1건/라인·작업자 journal 귀속" + `openBoxByScan→inspectScan` 자동완료 e2e 5개 GREEN 실증. 프로덕션 버그 0건, spec 하네스 결함 2건만 수정(`DbService.run` 대역 누락, `stockSummary` view 네임스페이스). 상세 = Immediate Next Step #6.
5. ✅ **해소 (2026-07-01 read-only 점검) — "FO 0건" 전제는 거짓이나 B 게이트는 클린**: live core 에 `fulfillment_orders` **10행** + `fulfillment_order_items` **28행** 존재(Cluster A 적용 시 발견). `invoices`/`shipments`/`inspection_*`/`stock_reservations`/`fulfillment_order_lines`(FOL) 는 **0행** → Cluster A 마이그레이션엔 무해(빈 자식테이블에만 컬럼 drop·enum 교체). **B 선결조건 read-only 점검 = 전부 GREEN**: (a) 중복 `sales_order_id` **0건**(`GROUP BY sales_order_id HAVING count>1` 공집합 → FO-split 흔적 없음), (b) SO-merge 흔적 **0건**(어떤 FO 도 여러 SO 의 FOI 가 섞이지 않음 — `FOI.sales_order_id` 전부 부모 FO 와 일치), 10건 모두 **서로 다른 SO 와 1:1**(`distinct_nonnull_so=10`, `null_so=0`). ⇒ **B 의 `fulfillmentOrders(salesOrderId) WHERE salesOrderId IS NOT NULL` 부분 unique 를 위반 없이 추가 가능 — 데이터 정리 불필요, B = 1 PR(클린) 유지.** FOL=0 이라 FOL drop 도, shipments/예약 0 이라 M:N 개방(`uqFulfillmentOrder` drop+`openedForFO` nullable)도 무해. (c) 성격 = **06-30(Cluster A 개발일) QA/테스트 잔재**: 전부 `status=unfulfillable`·`fulfillment_mode=null`·`total_reserved_qty=0`·`shipped=0`·예약 0·송장/박스 0, 실재 `sales_orders`(567행 중 일부) 참조(orphan 0), 06-30 하루에 산발 생성 → 프로덕션 출고 트래픽 아님. 선택적 정리 가능하나 B 에 불필요. ⚠️ **잔여 caveat(B6 동일)**: 이 B PR 배포 *전에* 출고가 프로덕션 라이브가 되면 신규 split/merge 가 위반자를 만들 수 있으므로 unique 추가 직전 동일 점검 재실행 필요. (점검 방식: `cd deployments/lcnine/services && npx sst shell --stage live -- npx tsx <read-only SELECT script>`. SSL 명시 필요 — `ssl:{rejectUnauthorized:false}`, URL 의 `uselibpqcompat` 만으론 postgres.js 가 SSL 누락해 ECONNRESET.)
6. ✅ **해소 (live 적용 시) — 마이그레이션 DDL ordering 버그 2건** (`b3cbb0113`): drizzle-kit 이 EU1 마이그레이션(`20260630125603`)에 심은 ordering 버그가 코드리뷰(SQL 정독)로는 안 잡히고 **live 적용에서야** 터짐 — (a) `DROP TABLE inspection_sessions CASCADE`(#4)가 이미 제거한 FK 를 뒤에서 중복 `DROP CONSTRAINT`(→ `IF EXISTS` 멱등화), (b) 부분 unique `uq_invoices_fo_active`(predicate `status<>'canceled'`)가 `invoices.status` 타입 변경(→text)을 막음 → `DROP INDEX` 를 enum 교체 *앞*으로 이동. **교훈(통합검증 빚 #4 의 실제 비용): destructive 마이그레이션은 live 적용 전 `BEGIN…ROLLBACK` 문장별 드라이런 필수** — 메모리 `feedback_migration_dryrun_before_live`.
