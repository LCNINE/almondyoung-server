...
| Phase 2 상자 워크플로 (클러스터 A) | 🟦 설계 확정 | 검수 영속 붕괴·invoice FK 뒤집기·자동완료 트리거·lazy 박스 생성. 구현 대기. 상세 = Resolved Decisions §Phase 2 (클러스터 A). |
...
8. ⏭️ **다음 = Cluster A 구현** (TDD, 1 PR): 검수 영속 붕괴(`inspection_sessions`/`items`→`shipment`/`shipment_line`, `inspection_issues` 유지)·`shipment_line{qty,inspectedQty,forced}`·검수 write-ownership 역전(`FOI.shippedQty` consume-driven)·`invoice.shipmentId` FK 뒤집기·자동완료 소진 트리거·박스 lazy 생성·명시적 ship 엔드포인트 은퇴. → 이어서 Cluster B(구조만: FOL/FO-split/SO-merge 제거 + FO↔상자 M:N 개방 + SO:FO 부분 unique).
9. 직배(drop-ship) 모델 추출 — 별도 ADR + phased plan. **B(SO:FO 1:1)의 선결조건 아님**(B5 — 혼합주문이 이미 단일 FO). 추출 이유 = 직배 품목이 자사 FO 로 흡수되는 잠재버그 분리.
