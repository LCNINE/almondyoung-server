...
### 등록·발행 및 가격/상세페이지 보완 (2026-09-14)

이 단계는 앞선 «초안만 저장» 범위를 확장한다.

- 초안의 `sales`에 시장가·공급가, 판매가·멤버십가, 옵션별 가격, 카테고리·대표카테고리, 운영 태그 ID, SKU 매칭 계획을 보존한다. 시장가/공급가는 버전 필드, 판매/멤버십 가격은 기존 가격 규칙 서비스에 저장한다. 미정 시장가/공급가는 기존 값을 지우지 않는다.
- `search_product_references`는 카테고리·SKU·태그 이름을 한 번에 20개까지 조회한다. 범용 HTTP/SQL/외부 실행 도구는 제공하지 않는다. 검색 후속 루프는 최대 5회, 각 모델 요청 20초, 전체 응답 150초, 클라이언트 스트림 180초로 제한한다.
- 사용자가 직접 «등록해줘», «이대로 발행해줘»라고 요청하면 필수 정보를 한 단계씩 확인한다. 유효한 등록 의도는 후속 답변까지 유지하되 초안/미리보기/취소 요청으로 해제한다. 중지/실패하여 assistant 답변이 없는 턴 뒤 새 입력도 이전 등록 의도를 이어받지 않는다.
- 준비 완료 시 AI 세션 소유권·최신 답변 lease와 기존 내 draft 소유권을 검증하고, 저장·가격·옵션·분류·재고 연결·`ProductVersionsService.publishVersion`·성공 메시지를 같은 DB 트랜잭션으로 처리한다. 발행 검증 실패/취소는 DB 변경과 성공 메시지를 롤백한다. 동일 응답 재시도는 중복 발행하지 않는다.
- 신규 카테고리/SKU 계획은 먼저 미리보기에 제시하고 그 이후의 직접 등록 요청으로 승인받는다. 초안 저장 버튼도 표시된 계획의 저장을 실행한다. 신규 SKU는 서버에서 `inventory.manage` 또는 master 권한을 별도로 검증한다. 새 SKU는 physical 품목 정의이며 입고 수량/무재고 판매 권한을 만들지 않는다. 실제 생성 ID를 초안에 반영하여 다음 저장에서 재생성하지 않는다.
- 옵션은 최대 3개 그룹·100개 조합, 조합당 SKU 링크 한 개를 지원한다. 여러 SKU로 구성된 세트, 기존 발행 상품 편집, 수량 입고/조정, 회원 전용 노출·구매 정책은 이 도구 범위에 포함하지 않는다.
- 상세페이지는 짧은 summary 대신 모든 섹션을 편집기 표준 Markdown과 `product-image` file-ID directive로 저장한다. HTML도 안전한 이스케이프 결과로 함께 보존한다. 기존 화면에서 요약이 HTML을 가리던 문제를 새로운 저장 경로에서 해결한다. 첨부 근거 없이 방수·소재·인증을 생성하지 않도록 지침을 명시한다.
- 기존 JSONB 컬럼을 확장하므로 추가 DB 마이그레이션은 없다. 기존 초안은 `sales`가 없어도 읽을 수 있다. 이전에 누락된 가격은 대화를 바탕으로 새 미리보기를 생성해 저장해야 하며 임의 역산하지 않는다.
- 외부 파일 복사와 기존 도메인 이벤트의 부수효과는 DB 트랜잭션 자체로 되돌릴 수 없다. 실패한 저장의 미사용 공개 이미지 정리는 후속 과제다. 실제 원격 쇼핑몰 발행은 테스트하지 않는다.

- 검증 완료: Core 빌드, admin 타입 검사, 관련 102개 테스트 통과. 실제 GPT 이미지 입력에서 네 종류 가격을 확인했고, 로컬 `[연결 검증] 냥이 스티커` 초안을 저장한 뒤 상품 조회 API에서 시장가 5,000/공급가 1,000 및 가격 규칙에서 판매가 3,000/회원가 2,500을 확인했다. 실제 상세설명 Markdown에 복사된 이미지 ID와 소개·특징·사용 안내가 들어갔으며 상태는 draft로 유지했다. 로컬 Core 서버 재시작 완료. 원격 배포/실제 판매 상품 발행은 수행하지 않았다.
