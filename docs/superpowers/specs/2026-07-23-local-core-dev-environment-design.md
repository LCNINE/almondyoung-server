...
| 화주 | 2개 | 전부 |
| SKU | 20개. 바코드 `DEV-SKU-0001…` 고정, 일부 sub-barcode 보유, 일부 배치/유통기한 관리 | 전부 |
...
SKU +300, 로케이션 +50, 판매주문 +50. 재고조회가 서버 페이지네이션 표라 규모가 있어야 페이지네이션·검색·정렬의 실제 감각을 볼 수 있다. 평소 리셋은 소형으로 빠르게 돌린다.
