...
- 구버전 앱: `tauri.conf.json` 에 자동 업데이트가 없어 구버전이 남을 수 있다. 설계 당시에는 라이브 예정 데이터가 없어
  기능 손실이 없다고 가정했지만, 이후 PR-B 적용에서 source 계획 182건·품목 2,533건이 확인되어 그 가정은 폐기됐다.
  설치된 구버전은 예정 입고 화면에서 404를 받아 해당 기능을 쓰지 못할 수 있다. 설치 현황, 실제 사용 여부, 업데이트
  상태는 이 작업에서 확인하지 않았다. 창고 앱 native binary는 SST 서버 배포 완료 기록의 적용 대상이 아니다.
