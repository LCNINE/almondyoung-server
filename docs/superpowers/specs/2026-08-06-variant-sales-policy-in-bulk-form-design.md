...
| key | label | 값 |
|---|---|---|
| `availabilityOverride` | 판매상태재정의 | 빈칸 = 변경 없음 / `품절` / `출시예정` / `없음` = 해제 |
| `comingSoonDate` | 출시예정일 | `YYYY-MM-DD` 또는 빈칸 |
| `preStockSellable` | 선판매 | `Y` / `N` / 빈칸 |
| `alwaysSellableZeroStock` | 항상판매 | `Y` / `N` / 빈칸 |

**해제를 빈칸이 아니라 `없음` 으로 두는 것이 이 열의 핵심 계약이다.** 빈칸=변경 없음은 이 양식 전체의 규약이고(`bulk-session.fields.ts:20-30`), 그것을 정책 열만 뒤집으면 정책 열을 안 채운 행 전부가 품절 해제 변경분을 만든다. 1,000행 양식에서 30건만 고치려던 파일이 나머지 970건의 품절을 푸는 사고다 — 카테고리 전량 해제(`bulk-session.fields.ts:73`)와 완전히 같은 종류이고, 그때와 같은 결론을 쓴다.
...
- `판매상태재정의` — `품절` · `출시예정` · `없음` · 빈칸 외의 값은 행 오류.
- `출시예정일` — `YYYY-MM-DD` 파싱 실패는 행 오류. **`판매상태재정의` 가 `출시예정` 이 아닌데 날짜가 있으면 행 오류**다. 서버는 `coming_soon` 이 아니면 날짜를 비우므로(`product-sku-mapping.service.ts:50-56`) 조용히 버려지고, 작업자는 자기가 넣은 날짜가 사라진 것을 모른다.
...
**추출기는 순수 함수다** — `FlatFields` 를 받아 `Map<조합키, StockPolicyPatch>` 를 돌려준다. 워크북 문자열(`품절`/`Y`/`없음`)을 DTO 값(`'manual_out_of_stock'`/`true`/`null`)으로 옮기는 판정이 전부 여기 모이고, 그래서 DB 없이 테스트된다.
