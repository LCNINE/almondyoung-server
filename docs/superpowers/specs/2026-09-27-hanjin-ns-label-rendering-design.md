...
상태: 설계 합의 완료, 구현 전. 마스킹 유틸(§4.1)만 먼저 구현돼 있다.
...
- 긴 텍스트(주소·배송메시지·품명)는 칸 폭을 넘으면 말줄임(`…`)으로 자른다. 폭 판정은 폰트 메트릭 없이
  «글자 수 × 칸별 상한» 으로 한다(칸별 상한은 실측 상수). 줄바꿈은 하지 않는다 — 선인쇄 칸을 넘는다.
...
3 이 있어서 **라벨의 수하인·품명은 한진에 등록한 값과 같다.** 라벨은 발급 때의 사본이 아니라 현재
shipment 로 다시 조립하는데, 버전·해시가 같으면 내용도 같다.
