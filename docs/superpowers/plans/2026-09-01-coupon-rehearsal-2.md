...
> 화면 항목(🖥️)은 크롬 자동화로 구동할 수 있으나 **판정은 사람이 한다** — 그 경계는 §7 에 있다.
>
> **▶ 세션을 시작할 프롬프트는 §7 에 복붙용으로 있다.**
...
**「구동」 열의 뜻** — 🖥️ 브라우저 · ⌨️ 터미널/curl · 🗄️ DB 조회 · **📸 = 스크린샷을 증거로 반드시 남긴다.**
📸 가 붙은 것은 **판정이 화면에 있는** 항목이다. 크롬 자동화로 돌리든 손으로 돌리든, 그 항목의 ✅ 는
스크린샷 없이는 근거가 없다(§7 참조). 1차에는 이 증거가 없었다.

| # | 구동 | 항목 | 무엇을 지키는가 | 자동 테스트가 왜 못 덮나 |
|---|---|---|---|---|
| **R1** | 🖥️ | 쿠폰 6종 생성 → 카트 적용 → 할인액 대조 | 폼→엔진 페이로드 | 축(`target_type`×`discountType`) 조합은 실 폼에서만 |
| **R2** | 🖥️ | 배송비 쿠폰 생성 | `F1` 회귀 | 1차의 유일한 ❌ 였다 |
| **R3** | 🖥️ 📸 | **배송비 쿠폰 + 정률 캡** | `A4`/P10-B | **배송옵션 픽스처가 저장소에 없다** |
| **R4** | 🖥️ 📸 | 캡이 표시되는가 (프리뷰·이벤트·체크아웃) | P10-B 표시 9곳 | `.tsx` 는 렌더 테스트가 없다 |
| **R5** | 🖥️ 🗄️ 📸 | `validity_days` → 링크행 `expires_at` | P4+P5 인스턴스 축 | 값의 «의미»는 화면에서만 보인다 |
| **R6** | 🖥️ 🗄️ | 정책 변경이 **소급 만료시키지 않는가** | P4+P5 핵심 결정 | 시간축 시나리오 |
| **R7** | 🖥️ 🗄️ 📸 | 발급창 밖 발급 거부 + **기존 보유분은 계속 사용 가능** | 결정 1의 존재 이유 | 엔진 캠페인 필터 우회의 실증 |
| **R8** | ⌨️ | `detach-coupon-campaigns.ts` dry-run → 반영 | 배포 후 필수 스크립트 | **라이브에서 처음 돌면 늦다** |
| **R9** | ⌨️ 🖥️ 📸 | 분류표 밖 룰 → 발급 거부 · 새 문구 · `force` 우회 | P7 `1-5` | 어드민 다이얼로그 문구는 `.tsx` |
| **R10** | ⌨️ 🖥️ | 카트 문맥 룰(`subtotal`) 쿠폰은 발급됨 | P7 대조군 | — |
| **R11** | 🖥️ ⌨️ 🗄️ | **자동발급 end-to-end — `customer_registered`** | `A5` 전체 | 라이브·로컬 통틀어 **실행 0회** |
| **R11b** | ⌨️ 🗄️ 🖥️ | **자동발급 end-to-end — `membership_activated`** | `A5` 두 번째 트리거 | 앱 4개를 가로지른다 |
| **R12** | ⌨️ | `/metrics` 에 발급 카운터가 뜨는가 | `7-4` | 스크레이프는 앱 밖 |
| **R13** | 🗄️ | 빠른 레인이 실패 행을 **1회만** 되살리는가 | `7-2` | 크론 실발화 |
| **R14** | 🖥️ 🗄️ 📸 | 전액 환불 후 쿠폰 상태 | `A2`(미해결 확인) | — |
...
🔴 **한 항목이 끝날 때마다 즉시 적는다.** 이 리허설은 앱 7개를 세우고 14항목을 도는 긴 작업이라,
마지막에 몰아서 쓰려다 중간에 끊기면 앞의 결과가 사라진다. 스크래치패드에 누적하고 끝에 옮긴다.
...
5. **📸 항목은 스크린샷을 코멘트에 붙인다.** 판정이 화면에 있는 항목의 ✅ 는 스크린샷 없이는 근거가 없다 — 특히 **브라우저 자동화로 돌렸다면 필수**다(§7의 위험 참조)
...
```
쿠폰 개통 리허설 2차를 진행한다. 사람이 손으로 돌리는 절차고, 나는 그 절차를 안내·구동·기록한다.

지침서: docs/superpowers/plans/2026-09-01-coupon-rehearsal-2.md
  — 이 문서 하나로 실행 가능하게 썼다. 먼저 전문을 읽어라.
  — 환경 절차의 정본은 docs/local-dev.md 「전체 스택 로컬 구동」이다. 지침서가 거기에
    channel-adapter(:3010)와 membership(:3001)을 더한다.

브랜치: feat/coupon-auto-issue-activation (PR #772, 미머지). 이 브랜치에서 돌린다.
  — develop + P7 이라 배포될 코드와 같다. 시작 전에 `git log --oneline HEAD..origin/develop`
    로 develop 이 움직였는지 보고, 움직였으면 리베이스한 뒤 그 해시를 결과에 적어라.
  — ❌ 가 P7 의 결함이면 이 브랜치에서 고친다. 그러라고 머지 전에 하는 것이다.

배경(필요하면 읽어라, 먼저 읽을 필요는 없다):
  - SoT = 이슈 #488. 1차 기록은 그 안 「리허설 1차 실행 기록」 절
  - 로드맵 = docs/superpowers/plans/2026-08-29-coupon-domain-master-plan.md

규칙:
  - R1~R14(R11b 포함) 전부 실행한다. 못 돌린 항목은 ⛔ 로 남기고 ✅ 로 반올림하지 않는다.
    1차가 「11항목 전부 실행, ⛔ 0건」을 지킨 것이 그 기록을 믿을 수 있게 만든 이유다.
  - 📸 표시 항목은 스크린샷을 증거로 남긴다. 그것 없이는 ✅ 의 근거가 없다.
  - ❌ 는 증상 + 요청/응답 + 해당 파일·줄까지 남긴다. 그 자리에서 조용히 고치지 말고 먼저 기록해라.
  - 🔴 한 항목을 끝낼 때마다 결과를 스크래치패드 파일에 «즉시» 적어라. 마지막에 몰아서 쓰지 마라 —
    이 리허설은 길고, 중간에 컨텍스트가 소진되면 앞의 결과가 통째로 날아간다.
    전부 끝나면 그 파일을 #488 코멘트로 옮긴다.
  - 결과는 #488 에 「리허설 2차 실행 기록」 코멘트로 남기고, 마스터플랜 진행 상황도 갱신한다.
  - 로컬 .env 는 커밋하지 않는다.

크롬 자동화를 쓴다 (체크리스트의 🖥️ 항목):
  - 시작할 때 claude-in-chrome 스킬을 먼저 부르고, tabs_context_mcp 로 컨텍스트를 잡은 뒤
    새 탭을 만들어 쓴다. 기존 탭을 재사용하지 않는다.
  - localhost:8000(스토어프론트)·localhost:8002(admin-web) 사이트 권한이 확장에 있어야 한다.
  - 네이티브 alert/confirm 을 띄우지 마라 — 뜨면 확장이 먹통이 되고 사람이 직접 닫아야 한다.
    admin-web 의 다이얼로그는 React 컴포넌트라 안전하지만, 삭제류 버튼은 누르기 전에 확인해라.
  - 2~3회 실패하면 같은 동작을 반복하지 말고 멈춰서 나에게 물어라.
  - 🔴 자동화는 «구동기»지 «심판»이 아니다. 화면의 숫자를 읽는 것과 그 숫자가 옳은지는 다른
    문제다. 판정이 애매하면 ✅ 로 넘기지 말고 스크린샷과 함께 나에게 확인을 요청해라.

가장 조용히 실패하는 지점 셋 — 막히면 여기부터 의심해라 (지침서에 상세히 있다):
  1. apps/medusa/.env 의 COUPON_AUTO_ISSUE_ENABLED=true 누락 → 발급 API 가 빈 배열만 반환
  2. channel-adapter 의 MEDUSA_MEMBERSHIP_GROUP_ID 누락 → inbox 는 published 인데 쿠폰이 안 나감
  3. Medusa customer 미존재(스토어프론트 첫 로그인 전) → 슬로우 리트라이로 최대 1시간 대기

리허설이 통과하면 그때 PR #772 를 머지하고, 개통(A5 플래그)은 그 뒤 별도 작업이다.
```

### 🔴 자동화가 늘리는 위험 — 이것만은 읽고 시작한다

브라우저 자동화는 **처리량을 올리는 만큼 «거짓 초록»도 올린다.** 에이전트가 페이지를 잘못 읽고
✅ 를 적으면 1차보다 **나쁜** 기록이 된다 — 1차는 적어도 사람 눈이 봤다.

그래서 📸 규칙이 있다. 스크린샷이 붙은 ✅ 는 나중에 반증 가능하고, 안 붙은 ✅ 는 아니다.
**자동화의 값어치는 속도가 아니라 그 증거에 있다.**

환경 구축(§2)은 자동화 대상이 아니다 — 앱 7개·env·시드는 터미널 작업이다.
