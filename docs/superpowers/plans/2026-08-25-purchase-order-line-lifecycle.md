...
8개 태스크를 태스크별 서브에이전트로 실행했다. 각 태스크마다 구현 → 리뷰 → (필요 시) 수정 라운드 →
범위 재리뷰를 돌리고, 마지막에 전체 브랜치 리뷰 1회 + 수정 웨이브 1회 + 범위 재리뷰 1회를 붙였다.
커밋 16개. 최종 게이트: `type-check` 0 · `jest` 4040 통과/실패 0 · DB 통합 102 통과.

## 계획서가 틀렸던 곳 (다음에 계획을 쓸 때 참고)

1. **통합 스펙 스니펫의 생성자 arity 를 지어냈다.** `new InboundService(dbService)` 라고 썼는데 실제로는
   6인자다. Task 1·2·4 에서 세 번 반복해 구현자가 매번 우회했다. 스펙 스니펫을 쓸 때는 대상 파일의
   생성자를 먼저 읽을 것.
2. **`inventory-scope-coverage.spec.ts` 의 존재를 몰랐다.** 새 라우트를 그 표에 등록하지 않으면 단위
   게이트가 빨개진다. 라우트를 추가하는 계획은 인가 스코프 표를 반드시 훑을 것.
3. **`expectedArrival` 의 계약을 스펙에서 연결하지 않았다.** 새 `date` 컬럼에는 `@Matches` 를 쓰라고
   적어놓고, **기존 헤더 필드가 같은 값을 같은 컬럼으로 흘려보낸다는 것**을 연결하지 않았다. 그 탓에
   검증이 세 라운드에 걸쳐 좁혀졌다 (`@IsDateString` → `@Matches` → 왕복 달력 검사).
4. **테스트가 "무엇을 판별하는가"를 명시하지 않았다.** 개수 단언은 옛 코드에서도 통과했고, 합계 단언만이
   실제 방어선이었다. 단언을 적을 때 "이 단언은 어떤 옛 동작에서 실패하는가"를 함께 적을 것.

## 판정 21건 (요약)

전체는 각 커밋 메시지와 이 문서 위쪽 태스크 본문에 있다. 성격별로:

- **수정 지시 15건** — 재확정 이중삽입, ABBA 교착, 심사 게이트 우회, 낡은 ETA, 부분 ISO 500,
  달력 검증, 잠금 순서, 테스트 판별력 등.
- **설계 구체화 3건** — CONFIRMED 분기를 라인 실행 경로로, 백필 ETA 보존, 이월 처리.
- **파킹 2건** — 아래 "남은 것" 참조.
- **되돌린 판정 1건** — 라운드 2에서 `@Matches` 로 좁히라고 지시한 것이 `@IsDateString` 의 범위 검사를
  버려 한 크래시 부류를 더 좁은 부류로 바꿨다. 왕복 검사로 교체.

## 남은 것 (파킹 — 사람이 정할 몫)

1. **`createInboundPlan` 에 `FOR UPDATE` 없음.** 같은 PO 로 동시 `POST /inbound/plans` 2건이 계획을
   2행 만들 수 있다 → 그 PO 전 SKU 의 `inbound_pending` 2배. 최종 재리뷰 권고는 "지금 넣어라"(1줄,
   잠금 불변식과 충돌 없음, 마이그 불필요). 도달성은 운영자 더블클릭뿐이고 해당 admin-web 화면은
   현재 별개 버그로 비활성이다.
2. **승인 후 라인 추가로 심사 우회.** 승인+확정된 발주에 새 라인을 추가하면 `auditStatus` 는 `approved`
   로 남은 채 그 라인을 즉시 실행할 수 있다. 진단 문서 발견 ②.3 그대로이고 **develop 대비 좁아진**
   상태다(종결 라인은 이제 보호된다). 닫으려면 D1(심사 워크플로 존치)을 구현으로 결정하게 되므로 보류.

## 잠금 불변식 (지우지 말 것)

**PO 행 → 라인 행 순서로만 잠근다.** 취득 지점이 셋이고(`purchase-order.service.ts` 2곳,
`inbound.service.ts` 1곳) 테스트가 없다 — 주석만이 재발 방어선이다. 최종 리뷰가 6개 경로 전부
준수를 확인했다. 여기에 PO 쓰기를 추가하는 편집은 잠금을 먼저 잡을 것.
