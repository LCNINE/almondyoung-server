...
- **후속 PR (contract phase, 배포 한 사이클 뒤) — 아래를 «한 PR 로 묶는다»** (최종 브랜치 리뷰 권고):
  ...
  6. ~~클레임 라우트의 200 응답 형태 2종 통일 (`{issued,reason}` vs `{success,promotion_id}`) — 오늘 스토어프론트가 본문을 안 읽어 안전하지만 스펙이 `reason` 을 단언한다~~ — **PR-2(`feat/coupon-module-depth`) Task 6 이 끝냈다**
