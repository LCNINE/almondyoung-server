...
- 1회 결제는 배너의 철회 버튼이 없다(되돌릴 정기결제가 없다). 서버가 미리보기의
  ...
  (`canResumeRecurring`). 둘 다 없는 옛 계약은 막지 않는다 — 근거 없이 CS 를 막는 쪽이 더 나쁘다.
...
# 관리자 UI — 실제 크로미움, 5 시나리오
cd apps/admin-web && npm run test:e2e:membership-cancel
```

- 서비스 계층(74): 상태 전이·자격·계약이벤트·더닝·재청구 차단, 관리자 해지예약 대행, 1회 결제 철회 차단
...
- UI: 스텁 백엔드로 대체해 실제 화면 조작 (`e2e/membership-cancel/`).
  관리자 5 시나리오(`annual monthly-cms scheduled one-time one-time-scheduled`), 고객 6 시나리오
