...
- **즉시해지와 해지예약을 함께** 보여준다(`status=CANCELLED_ANY`). 예전에는 `CANCELLED` 만 조회해
  **고객이 해지 신청한 건(잔여기간 이용 중)이 목록에서 통째로 빠졌다.** 행마다 `즉시 해지`/`해지 예약`
  배지가 붙고, 해지일은 예약 건이면 `recurringCancelledAt` 을 쓴다(예약 건은 `cancelledAt` 이 비어 있다).
