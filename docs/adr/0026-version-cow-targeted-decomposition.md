...
- **단 하나, 진짜로 동일한 프리미티브 — 고아 정리 — 만 추출한다.** "이 entity 를 가리키는 정션이 0개면 entity 삭제"가 **4개 파일에 7벌** 존재한다(variant 2, pricing rule 2, purchase constraint 3; 그중 `product-masters.service.ts:1046-1058` 은 메서드 추출조차 안 된 인라인). `product-masters.service.ts:1898` `_cleanupOrphanedVariants` 는 "남은 매핑이 현재 draft 뿐이면 삭제" 라는 status-join 분기가 더 있어 같은 프리미티브가 아니므로 **제외**한다(통합하면 동작 변경 또는 predicate-hook 과추상화). 7벌을 순수 함수 하나로 통일한다:
...
- 고아 정리 7벌 → 순수 함수 1개. variant 카운트의 masterId 필터 불일치 제거. 호출부는 `product-versions`·`product-masters`·`pricing`·`product-purchase-constraints` 네 서비스.
