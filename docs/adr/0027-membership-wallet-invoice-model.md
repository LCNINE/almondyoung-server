...
**이 절은 구현하면서 두 군데가 바뀌었다. 바뀐 쪽이 코드다.**

- **① 차단이 아니라 «선지급 금지»다.** 가입도 결제수단 등록도 그대로 허용한다 — 막으면 갚을 길까지
  막혀 빚이 영영 회수되지 않는다. 대신 **자격을 미리 주지 않는다**: 미수가 있는 계정은
  `grantAdvanceEntitlement` 를 건너뛰고, 수금이 확인되면(`invoice.paid`) 그때 자격이 붙는다.
