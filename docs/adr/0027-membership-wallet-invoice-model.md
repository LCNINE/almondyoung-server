...
CMS_BATCH 삭제는 효성 `deleteMember`로 약정을 지운다. 위험은 "등록계좌 삭제" 자체가 아니라 **로컬 뷰가 CMS와 어긋난 채**(우리는 FAILED/PENDING인데 CMS는 REGISTERED) 삭제되는 것이다. 따라서 **로컬 상태가 FAILED/PENDING인 삭제에 한해** `deleteMember` 직전 CMS를 재조회하고, 실제 등록완료로 확인되면 삭제를 중단하고 불일치를 사용자에게 명시(재확인 요구)한다. 로컬이 이미 REGISTERED인 정상 삭제는 기존 상태별 confirm으로 충분하며, 재확인을 하드 게이트로 걸지 않는다(CMS 점검 중 정당한 삭제가 막히는 것을 방지). CMS 재조회 실패 시에도 하드 블록하지 않고 경고 후 진행한다.

> "REGISTERED/처리중이면 차단"으로 문자 그대로 해석하면 정상 삭제(은행 교체를 위한 등록계좌 해지, 심사중 취소)까지 막혀 틀린 가드가 된다. 되돌릴 수 없는 피해는 딱 하나 — 살아있는 약정을 사용자가 "실패/심사중인 줄 알고" 지우는 것 — 이고, 그건 stale 발산 구간(로컬 FAILED/PENDING × CMS REGISTERED)에서만 발생한다. 유태림 케이스는 stale 발산이 아니라 진짜 실패건 삭제라 무해했으므로, §5-4는 아직 발생하지 않은 시나리오에 대한 안전벨트다. 근본 방어는 상태 정확성 + 접수/최종 구분 UX(§5-3)이고, 여기에 매-삭제 CMS 재조회 레이턴시·장애 취약점을 얹지 않는다.
...
**이 절은 구현하면서 세 군데가 바뀌었다. 바뀐 쪽이 코드다.**

- **① 선지급 금지.** 미수가 있는 계정은 `grantAdvanceEntitlement` 를 건너뛰고, 수금이 확인되면
  (`invoice.paid`) 그때 자격이 붙는다. 활성 계약을 가진 채 미수가 생긴 계정의 갱신은 이것만 막는다.
- **①-2 새 가입은 막는다(가입 관문, `ArrearsGate`).** 처음엔 「막으면 갚을 길까지 막힌다」며 가입을 허용했는데,
  그러면 재가입이 새 달 요금만 내고 옛 빚은 둔 채 다시 쓰는 길이 된다. 지금은 갚을 길이 가입과 따로 있다
  (자가 청산 `POST /me/arrears/checkout`) — 그래서 막아도 회수 경로는 열려 있다. 빚을 «청구»하는 게 아니라
  «갚기 전엔 새 이용을 개시하지 않는» 것이다. 결제 «전» 세 곳(등록 수단 가입 · 1회결제 결제창 생성 ·
  약관 동의 기록 = 자동이체 계좌를 등록하러 떠나기 전)에만 서고, 결제가 끝난 뒤(confirm·capture)엔
  서지 않는다 — 거기서 막으면 돈은 받고 가입은 거절한다. 결제수단 등록·상품 구매는 막지 않는다.
  관리자 직접 등록·무상 부여·cafe24 자동 부여도 지나가지 않는다(관리자가 사정을 알고 준다).
- **② 게이트의 축은 `subscriberRef`(계약)가 아니라 «계정»이다.** 계약 단위로 세면 재가입이 새 계약을
  만들면서 옛 빚과의 연결이 끊겨 그대로 통과한다. 그래서 미수는 wallet 조회가 아니라 **membership 의
  `membership_arrears` 원장**이 소유한다 — wallet `invoices` 에는 `user_id` 가 없고, 있는 게 맞지도
  않다(wallet 은 의도적으로 도메인을 모른다).
- **원장에 한 줄이 생기는 조건**: 터미널 실패(`UNCOLLECTIBLE` / `MANDATE_REJECTED`) **이면서** 그 계정이
  해당 주기의 자격을 실제로 들고 있었고, **그 주기가 청약철회 대상이 아닐 때**. 청약철회 대상(자격 개시
  7일 이내 · 멤버십 혜택 미사용)은 돈을 냈다면 전액 환불받았을 주기라, 거기에 미납을 적으면 청약철회권과
  정면으로 부딪힌다. 판정은 해지 화면과 같은 함수(`isWithdrawalEligible`)를 쓴다 — 둘이 갈리면
  「돈을 낸 사람보다 안 낸 사람이 불리하거나 유리한」 경우가 생긴다. 그 밖의 주기는 혜택을 실제로
  썼는지와 무관하게 주기 요금 전액을 적는다(정상 납부자도 그 주기는 환불받지 못한다).
- **기존 계약에는 적지 않는다.** 미납 요금은 기존 회원에게 불리한 약관 조항이라, 그 약관에 동의하고 들어온
  계약에만 적는다(`TermsRulesReader`). 적지 않은 이유는 계약 이벤트 `ARREARS_SKIPPED` 로 남긴다.
- **심사 기한초과는 별도 상태가 아니다.** `MANDATE_TIMEOUT` 은 `MANDATE_REJECTED` + `metadata.lastErrorCode`
  로만 구분된다. 원인을 3값 enum 으로 만들면 한 값이 영구히 0건이 된다.
- **transient 구분(중요)**: `MANDATE_PENDING`/`ATTEMPTING`은 **미수가 아니다**(정상 대기). 게이팅은
  터미널 실패에만 건다. 안 그러면 정상 1~2일 승인 대기 고객이 신용불량으로 오분류.
- **아직 안 닫힌 것**: 심사가 «승인되기 전»에 자격을 먼저 주는 것 자체는 그대로다. 미수 전력이 없는
  초범은 이 게이트에 걸리지 않는다. 닫으려면 자격 부여를 CMS 승인 훅으로 옮겨야 하고, 그 대가는
  정상 신규 가입자의 자격이 심사 기간만큼 늦어지는 것이다 — 별건으로 판단한다.
