...
- Medusa projection inbox worker 는 batch-size 기반 throttle 이 아니라 task-local handler concurrency 와 handler start interval 로 외부 API 압력을 제한한다.
- Medusa projection inbox worker 는 명시적인 eventType allowlist 에 포함된 projection 이벤트만 claim 하며, row claim 은 atomic claim + lease 방식으로 중복 처리와 crash-stuck row 를 줄인다.
- `INBOX_MAX_CONCURRENT_HANDLERS` 는 task-local limit 이므로 서비스 전체 동시성은 ChannelAdapter running task 수와 곱해진다.
