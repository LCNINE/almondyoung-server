...
- 출고 종결의 물리 단위는 **상자 = 송장(라벨) 한 장**이다 (상자 라벨 스캔 = 출고 처리).
...
1. **출고 종결(재고 소진)의 단위는 상자(shipment)** = 송장 한 장. 상자 라벨 스캔이 종결 트리거다.
2. **출고주문(FO)은 박스가 아니라, 판매주문을 상품매칭 규칙으로 재고상품(SKU)으로 변환한 스냅샷**이다. SO ↔ FO = 0..1 : 0..1 (디지털-only 주문은 FO 없음, 보상출고 등 단독 FO 는 SO 없음).
3. **FO ↔ 상자 = M:N.** `shipment_line {shipmentId, fulfillmentOrderItemId, skuId, qty}` 이 source FOI 를 참조한다. 한 FOI 라인이 여러 상자에 걸치면 송장분할, 한 상자에 여러 FO 의 라인이 모이면 합배송 — 같은 모델로 표현.
4. 상자 스캔 시 각 상자 라인에 대해 **단일 SHIP 이벤트를 원장에 append**(on_hand 차감)하고 예약을 **소진(consume)** 한다. 차감할 로케이션은 **교체 가능한 전략**이 정한다 — 지금은 FIFO(`allocation-strategy` 재활용), 나중에 토탈피킹이 잡은 실제 로케이션.
