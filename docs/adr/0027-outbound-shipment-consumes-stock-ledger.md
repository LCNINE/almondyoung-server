...
**Amended 2026-07-18** — 본문·Consequences 의 운송장 레이어 명칭 `invoice`/`invoices` 는 이후 **waybill**(`waybills` 테이블, `dispatch_attempts.waybill_id`) 로 개명되었다. 모델(상자 1:N 발급 이력, active 1)은 그대로다. 이벤트 페이로드의 `invoice` 필드명만 와이어 호환을 위해 잔존.
