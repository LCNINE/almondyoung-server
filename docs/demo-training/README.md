...
이 문서는 demo 환경에서 발주, 입고, 적치, 주문, 출고를 연습할 때 사용합니다. 실제 운영 환경에서는 사용하지 않습니다.
...
5. 기존 주문, 발주, 재고를 삭제하거나 전체 데이터를 초기화하지 않습니다. 반복 실습에는 새 보충 작업을 사용합니다.

## Demo 데이터에서 알아둘 점

- 상품명, SKU, 바코드, 공개 상품 이미지는 기준 시점의 상품 정보를 demo로 복사한 것입니다.
- 가져온 원본 SKU에는 공급처 구매 관계가 없었습니다. demo에서는 발주 실습을 위해 이 SKU들에 `데모 국내 공급사` 관계를 보완했습니다. 화면의 demo 공급처와 단가는 실제 구매처나 계약 조건이 아닙니다.
- demo의 재고, 수요 이력, 발주, 주문, 고객, 배송 이력은 연습용입니다. 실제 운영 주문과 재고를 복사한 것이 아닙니다.
- 일부 원본 SKU에는 바코드가 없습니다. 바코드가 없는 품목은 SKU 코드로 검색하고, 스캔 실습에는 바코드가 표시되는 다른 품목을 선택합니다.
