...
[문서 목록](README.md) · [물류팀](warehouse.md) · [교육 안내자](operator.md) · [합동 실습](workshop.md)

## 시작 조건

- 관리자 화면에 `demoadmin`으로 로그인합니다.
- 상단에 demo 안내 문구가 있는지 확인합니다.
- 교육 담당자에게 대상 상품명, SKU 코드, 발주 수량, 목적지 창고를 받습니다.
- 이번 발주는 기본적으로 `국내` 유형을 사용합니다.
- 화면의 `데모 국내 공급사`는 실습용 관계입니다. 실제 구매처나 실제 단가로 사용하지 않습니다.
...
3. 대상 행의 상품명과 SKU 코드를 교육 담당자가 준 값과 비교합니다.
4. 다음 열을 읽습니다.
...
6. 발주 수량을 교육 담당자가 지정했다면 화면 제안 수량과 차이가 있는지 기록합니다. 실습 지시 없이 제안 수량을 임의로 바꾸지 않습니다.
...
- 자유 선택 상품으로 발주 추천을 연습하려면 교육 안내자에게 해당 상품의 수요 이력 준비를 요청합니다. 재고가 없는 상품은 `상품마다 준비할 수량`을 1로 두고 수요 이력을 준비합니다.
- 이미 재고가 충분하면 전체 초기화를 요청하지 않습니다. 다른 상품을 선택하거나 해당 상품을 출고한 뒤 다시 확인합니다.
- 제안을 불러오지 못하면서 판매 창고 관련 오류가 표시되면 창고 구성을 교육 안내자에게 확인받습니다.
...
화면 예시는 별도 demo 상품입니다. 현재 실습의 SKU와 수량은 교육 담당자가 지정한 값을 사용합니다.

6. 수량을 바꿀 때는 1 이상의 정수를 입력하고 입력이 저장될 때까지 기다립니다.
7. 필요 없는 항목은 휴지통 버튼으로 삭제합니다. `전체 비우기`는 현재 카트의 모든 항목을 지우므로 교육 담당자의 지시가 있을 때만 사용합니다.
8. `발주 생성`을 누릅니다.
9. `장바구니로 발주 생성` 창에서 다음 값을 입력합니다.
   - `공급처 *`: 대상 행에 표시된 demo 공급처
   - `목적지 창고 *`: 교육 담당자가 지정한 국내 demo 창고
...
보충 제안에서 카트를 사용하는 것이 기본 절차입니다. 교육 담당자가 정확한 내부 SKU ID를 준 경우에만 직접 생성합니다.

1. `재고관리` → `발주관리`에서 `발주 생성`을 누릅니다.
2. `발주 유형`은 `국내`를 선택합니다.
3. `입고 예정일`, `공급처 *`, `목적지 창고 *`를 입력합니다.
4. `발주 라인 *`의 `SKU ID`에 화면용 SKU 코드가 아니라 교육 담당자가 준 내부 SKU ID를 입력합니다.
5. `수량`은 1 이상의 정수로 입력합니다. `단가`는 실습값이 있을 때만 입력합니다.
...
정상 결과: 목록의 `라인 진행`이 `N/N 실행`으로 바뀝니다. 물류팀이 입고할 수 있도록 전체 발주번호, 목적지 창고, SKU, 실발주 수량, 도착 예정일을 전달합니다.

## 6. 부분 입고와 완료 상태 확인

물류팀이 입고를 확정하면 같은 발주 상세를 다시 엽니다.

- 일부만 받았으면 `받음 4 (남음 6)`처럼 표시됩니다.
- 전량을 받았으면 `전량 입고`가 표시됩니다.
- 모든 발주 라인의 입고가 끝나면 발주서 운영 상태가 `입고완료`로 바뀝니다.
- 입고 확정 뒤에도 물류팀의 적치 작업은 남아 있을 수 있습니다. `입고완료`만 보고 재고가 보관 위치에 적치됐다고 판단하지 않습니다.

![4개 입고 뒤 받음 4와 남음 6이 표시된 발주 상세](assets/screens/purchase-order-partial.png)

부분 입고 실습에서는 첫 입고 뒤 남은 수량을 물류팀에 다시 전달합니다. 두 번째 입고 뒤 `남음 0`과 적치 완료를 각각 확인합니다.
