...
[문서 목록](README.md) · [창고 앱 안내](warehouse.md) · [개발팀](operator.md) · [합동 실습](workshop.md)
...
6. 발주 수량을 개발팀이 지정했다면 화면 제안 수량과 차이가 있는지 기록합니다. 실습 지시 없이 제안 수량을 임의로 바꾸지 않습니다.
...
- 자유 선택 상품으로 발주 추천을 연습하려면 개발팀에 해당 상품의 수요 이력 준비를 요청합니다. 재고가 없는 상품은 `상품마다 준비할 수량`을 1로 두고 수요 이력을 준비합니다.
...
화면 예시는 별도 demo 상품입니다. 현재 실습의 SKU와 수량은 개발팀이 지정한 값을 사용합니다.

6. 수량을 바꿀 때는 1 이상의 정수를 입력하고 입력이 저장될 때까지 기다립니다.
7. 필요 없는 항목은 휴지통 버튼으로 삭제합니다. `전체 비우기`는 현재 카트의 모든 항목을 지우므로 개발팀의 지시가 있을 때만 사용합니다.
8. `발주 생성`을 누릅니다.
...
부분 입고 실습에서는 첫 입고 뒤 남은 수량을 입출고 담당자에게 다시 전달합니다. 두 번째 입고 뒤 `남음 0`과 적치 완료를 각각 확인합니다.
