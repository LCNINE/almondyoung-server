...
[문서 목록](README.md) · [리테일팀](retail.md) · [물류팀](warehouse.md) · [합동 실습](workshop.md)
...
상단 준비 상태는 고정 교육 세트 기준입니다. `전체 상품 기준정보`는 가져온 전체 상품 범위를 보여줍니다. 두 수치를 같은 의미로 해석하지 않습니다.

## 2. 실습 상품 찾기

1. `주문 생성 · 실습 상품 준비`의 `실습 상품 검색`에 상품명, SKU 또는 바코드를 입력합니다.
2. `검색`을 누릅니다.
...
3. 보충 제안 실습도 할 경우 `발주 추천용 수요 이력도 준비`를 선택합니다.
4. `선택 상품 보충`을 누릅니다.
5. 결과가 확정되지 않았다는 안내가 나오면 새 보충을 시작하지 말고 `보충 요청 재확인`을 누릅니다. 같은 요청은 재고를 두 번 더하지 않습니다.
6. `실습 재고 준비 완료`에서 창고, 위치, 입고번호, SKU별 증가 수량을 기록합니다.

발주 추천 실습에는 재고가 없는 상품을 선택하고 준비 수량을 1로 시작합니다. 이미 재고가 충분해 제안이 나타나지 않으면 재고를 초기화하지 않습니다. 다른 상품을 선택하거나 해당 상품을 출고한 뒤 다시 확인합니다.

입고와 적치 자체를 교육하려는 경우에는 이 보충 버튼을 쓰지 않습니다. [리테일팀 문서](retail.md)와 [물류팀 문서](warehouse.md)의 발주·입고 절차를 사용합니다.
...
기본 연습값은 주문 수 5, 주문당 상품 종류 2, 품목당 최소 1, 최대 3입니다. 현장 교육 시간이 짧으면 주문 수를 1로 줄일 수 있습니다.
...
4. 위치별 출고 실습은 `피킹 방식`에서 `개별 피킹`을 선택합니다. 다른 방식은 교육 담당자가 별도로 지정한 경우에만 사용합니다. `멀티오더 피킹 (바구니 카트)`을 지정받았다면 `카트 바구니 수`에 이 배치에 넣을 운송장 수 이상의 정수를 입력합니다.
...
9. 물류팀에 배치 ID, 작업 ID, shipment ID, 운송장번호, 창고, SKU와 수량을 전달합니다.

`Eligible shipment`에 대상이 없으면 운송장 발급 여부, shipment 상태, 배치 창고가 모두 맞는지 확인합니다. `서버 확인 중…` 뒤 결과가 불명확하고 버튼이 `원래 명령 재시도`로 바뀌면 새 배치를 만들지 말고 그 버튼으로 같은 명령을 재확인합니다. 위치별 출고 실습에서는 관리자 화면의 `계획 생성`, `Picker claim`, `Packer claim`을 누르지 않습니다. 물류 앱에서 `출고 준비`를 누르면 이 작업에 필요한 준비와 작업자 할당이 진행됩니다.
...
- DB 전체 초기화
- 기존 주문, 발주, 입고, 출고 작업 삭제
- 현재 재고를 임의의 기준 수량으로 덮어쓰기
- 결과가 불명확한 요청을 새 요청으로 다시 만들기
- demo 공급처나 단가를 실제 구매 정보로 전달하기
