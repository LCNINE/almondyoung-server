...
이 기능은 재고를 늘리므로 수요 이력을 함께 준비해도 발주 제안이 나오지 않을 수 있습니다. 발주가 필요한 상품을 새로 준비하려면 아래 `발주 제안용 데이터 준비`를 사용합니다.

입고와 적치 자체를 실증하려는 경우에는 이 보충 버튼을 쓰지 않습니다. [발주 안내](retail.md)와 [창고 앱 안내](warehouse.md)의 발주·입고 절차를 사용합니다.

## 발주 제안용 데이터 준비

![발주 제안용 데이터 준비 화면](assets/screens/demo-replenishment-form.png)

1. 시연 콘솔에서 `발주 제안용 데이터 준비`로 이동합니다.
2. `발주 제안 상품 선택`을 `무작위 실제 상품`으로 둡니다.
3. `무작위 상품 수`를 입력합니다. 기본 5개이며 1~20개까지 가능합니다.
4. `발주 제안 만들기`를 누릅니다.
5. 결과 표에서 상품명·SKU, 수요 유형, 일평균 수요, 현재고·미입고 수량, 재주문점, 발주 제안 수량을 확인합니다.
6. `보충 제안에서 확인하고 발주하기`를 눌러 [발주 안내](retail.md)의 절차로 이어갑니다.

![실제 상품 5개의 발주 제안 생성 결과](assets/screens/demo-replenishment-result.png)

이 기능은 수요·재고·거래 이력이 없는 실제 상품을 추첨하고, 최근 365일의 **시연용 합성 수요**를 준비합니다. 실제 판매 이력이 아닙니다. 꾸준한 수요, 변동이 큰 수요, 간헐적 수요를 섞으며 기존 발주 계산과 최소 발주 수량·포장 단위를 적용합니다. 현재고는 0인 상태를 유지하고, 1~1,000개 범위의 발주 제안이 실제로 나오는 상품만 생성 결과에 포함합니다.

특정 상품을 쓰려면 위 검색 결과에서 상품을 체크한 뒤 `위에서 직접 선택한 상품`을 선택합니다. 세트 상품은 중복을 제외한 구성 SKU가 최대 20개여야 합니다. 이미 작업이나 수요 이력이 있는 상품은 이 기능의 대상에서 제외됩니다. 사용할 수 없는 선택 상품이 있거나 요청한 수만큼 제안을 만들 수 없으면 해당 요청의 데이터 변경은 모두 취소됩니다.

무작위 모드는 위의 상품 체크와 관계없이 전체 후보에서 새로 추첨합니다. 생성 후 같은 버튼을 다시 누르면 다른 새 상품을 준비합니다. 통신 오류로 결과가 불명확하면 `발주 제안 요청 재확인`을 누릅니다. 같은 요청은 상품을 다시 추첨하거나 수요 이력을 중복 생성하지 않습니다.

발주를 확정하고 실제 발주 수량을 등록하면 미입고 수량이 보충 계산에 반영됩니다. 충분한 수량을 발주한 상품은 추가 발주 제안이 사라집니다. 결과 표는 생성 시점의 기록이므로 이후 상태는 보충 제안 화면에서 확인합니다.
