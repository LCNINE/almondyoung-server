...
[문서 목록](README.md) · [발주 안내](retail.md) · [창고 앱 안내](warehouse.md) · [개발팀](operator.md)

발주 담당자, 입출고 담당자, 개발팀이 같은 식별자를 사용해 한 작업을 끝까지 처리합니다. 각 단계가 끝날 때 다음 담당자에게 화면의 실제 번호를 전달합니다.

## 시작 전 확인

- 발주 담당자와 개발팀은 관리자 화면에 `demoadmin`으로 로그인합니다.
- 입출고 담당자는 Windows 창고 앱에 `demoworker`로 로그인합니다.
- Windows 설치 파일은 개발팀이 별도로 전달합니다.
...
## 예시 1. 국내 발주 10개, 부분 입고 4개와 6개

### 발주 담당자

1. [발주 안내](retail.md)의 `보충 제안 확인` 절차로 대상 SKU를 찾습니다.
...
7. 발주번호, SKU, 실발주 10, 목적지 창고, 예정일을 입출고 담당자에게 전달합니다.
...
1. [창고 앱 안내](warehouse.md)에 따라 `입고 예정`에서 공급처, 예정일, SKU, 남은 수량이 맞는 항목을 엽니다. 목록 카드에는 전체 발주번호가 표시되지 않으므로 번호만으로 선택하지 않습니다.
...
7. 화면에 표시된 입고번호나 적치 작업 ID가 있으면 함께 기록합니다. 표시되지 않으면 입고 시각, SKU, 원위치, 목적지 로케이션, 입고·적치 수량 4를 발주 담당자에게 전달합니다.

### 발주 담당자 확인

1. 발주 상세를 다시 엽니다.
2. 대상 라인에 `받음 4 (남음 6)`이 표시되는지 확인합니다.
3. 남은 6개를 입출고 담당자에게 다시 전달합니다.
...
- 발주 담당자: 라인이 `전량 입고`, 발주서가 `입고완료`인지 확인합니다.
- 입출고 담당자: 두 입고의 미적치 수량이 0인지 확인합니다.
...
1. [개발팀 문서](operator.md)에 따라 시연 데이터가 `준비 완료`인지 확인합니다.
...
9. shipment와 같은 `창고`를 선택하고 위치별 출고 예시의 `피킹 방식`은 `개별 피킹`을 선택한 뒤 `생성`을 누릅니다.
...
12. 배치 ID, 작업 ID, shipment ID, 운송장번호, 창고, 상품별 SKU와 수량을 입출고 담당자에게 전달합니다.
...
1. [창고 앱 안내](warehouse.md)에 따라 `출고작업`에서 전달받은 운송장번호를 스캔하거나 입력하고 `조회`를 누릅니다.
...
8. shipment ID, 운송장번호, 각 SKU 출고 수량, 완료 시각을 개발팀에 전달합니다.
...
- 발주와 입고를 함께 진행할 때: 발주 담당자가 새 발주를 만들고 입출고 담당자가 입고·적치합니다.
- 출고 단계만 확인할 때: 개발팀이 시연 콘솔의 `선택 상품 보충`을 사용합니다.

보충 뒤에는 새 주문을 만들지 않습니다. 기존 주문의 예약과 출고 준비를 다시 확인해 같은 주문을 계속 처리합니다.

예약이 완료되면 [개발팀 문서](operator.md)의 `물류 앱에 보낼 출고 작업 준비` 절차로 같은 shipment의 계획을 확정하고 운송장을 발급한 뒤 출고 배치에 추가합니다.

### 완료 확인

- 같은 주문번호와 shipment ID를 유지했는지 확인합니다.
- 부족 예약이 해소됐는지 확인합니다.
- 입출고 담당자가 위치별 상품 스캔을 끝내고 화면에 `출고완료`가 표시되는지 확인합니다.
...
3. 개발팀은 `같은 요청 재확인` 또는 `보충 요청 재확인`을 사용합니다.
4. 발주 담당자는 발주 목록과 상세에서 기존 발주번호의 상태를 다시 확인합니다.
5. 입출고 담당자는 화면에 작업 ID가 있으면 그 값을 확인합니다. ID가 없으면 상품, 작업 시각, 수량, 위치를 기준으로 입고·적치·출고 이력을 다시 확인합니다.
