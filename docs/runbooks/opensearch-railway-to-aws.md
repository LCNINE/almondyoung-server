...
2026-09-16 14:07~14:35(KST), Railway 인스턴스가 28분 죽어 **고객 검색이 전면 0건**이 됐다.
검색 페이지는 200 + 「검색 결과가 없습니다」로 떠서 상태 코드로는 보이지 않았다. 그 인스턴스는
백업도 인증도 없는 공개 엔드포인트였고, 검색 이력의 유일본이었다.
...
| 사양 | (Railway 요금제) | t3.small × 1, gp3 10GB, 월 약 $42 |
...
죽으면 이번과 같은 전면 중단이 다시 가능하다. 이중화하려면 `instanceCount: 2` +
`zoneAwarenessEnabled` 로 올린다(월 약 +$42).

## 전제

- `develop` 에 `[search] OpenSearch 장애 시 부팅 유지와 자동 재연결`(`ae0ef2d41`)이 들어가 있을 것.
...
라이브라 복사하는 동안에도 문서가 쌓인다(실측: 몇 분 사이 상품 +1, 검색이벤트 +63).
**컷오버가 끝난 뒤 한 번 더 돌린다.** 멱등이라 전체를 다시 훑고 바뀐 것만 덮어쓴다.
