...
| **정상통관 해외배송** — 재고 보유 | 마스트 드로잉 볼펜 카트리지 (13548, 13559) | **유지**  | 선판매(마스트 전부 대상, ②-C) |

**볼펜 카트리지는 제외하면 안 된다** (2026-08-04 확인). 공급처는 중국이지만 정상통관으로 들여와 재고를
쌓아두고 파는 상품이라, 재고연동이 끊기면 실물 없는 재고로 팔리거나 반대로 있는 재고가 품절로 굳는다.

**선판매는 켠다** (2026-08-06 정책 변경, 그 전엔 "켜지 않는다" 였다). 마스트 해외상품은 종류 불문 전부
선판매 대상이다(②-C). 재고가 있는 동안은 어차피 플래그가 발동하지 않아 재고대로 팔리고, 재고 0 이 되면
품절로 굳는 대신 선판매로 넘어간다. **동기화 제외(Ⓐ-0)와는 별개다 — 제외는 여전히 걸지 않는다.**
...
> ⚠️ **②③(입고예정) 보다 먼저 돌려야 한다.** ②가 바코드로 `sku_barcodes` 를 조회하는데, 신규 SKU 는 `import-products` 가 만들어야 존재한다.
...
```bash
# 해외 CSV(한국·한국직배 제외본, ②-B 참조) 로 돌린다. 종류 필터 없음.
NAME_FILTER='마스트|MAST' \
  bash scripts/sellmate/run.sh live set-sellable-policy <해외 csv> --flag pre_stock_sellable --on
# … 확인 후 --apply --out mast-variants.txt, 이어서 recalc-sellable
```

**이미 선판매로 되어 있는 품목은 무시한다.** dry-run 이 `이미 ON` 으로 찍는 건 손댈 게 없다는 뜻이지
빠뜨린 게 아니다. 품절 상태로 보이더라도 플래그가 ON 이면 그대로 둔다 — 재고 0 인데 품절로 보이는 건
플래그 문제가 아니라 매칭/캐시/예약 쪽이고, 진단은 아래 ⚠️ 두 절을 따른다.
...
| 브랜드/범위                       | 플래그               | 적용일     | 비고                                                                             |
| --------------------------------- | -------------------- | ---------- | -------------------------------------------------------------------------------- |
| 마스트(MAST) 머신·서플라이·배터리 | `pre_stock_sellable` | 2026-07-23 | 대상 77품목 중 **매칭된 7개만** 실제 적용 (나머지 70개는 미매칭이라 이미 판매중). 아래 행으로 범위 확대됨 |
| **마스트(MAST) 해외상품 전부** (부속류 포함) | `pre_stock_sellable` | 2026-08-06 | 종류 불문. 이미 ON 인 건 건드리지 않는다 |
