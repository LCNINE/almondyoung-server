...
### 발주 (Purchase Order) · 입고 계획 (Inbound Plan) · 이동 지시서 (Transfer Order) *(결정됨 — ADR-0032)*
- **발주**: 공급사에게 물건을 사는 문서. 소유 구간은 **공급사 → 출발 창고 도착**이고 거기서 `received` 로 종결한다. 해외 발주에서 출발 창고는 중국 물류창고이므로 **발주의 `received` 는 "중국 창고 입고 완료" 를 뜻한다** — 최종 목적지(부천) 도착이 아니다.
- 발주는 **라인 단위로 실행**된다. 발주서를 만든 뒤 직원이 라인을 하나씩 사고, 그 순간 수량·단가·ETA 가 확정되며 아예 못 사는 라인(`unavailable`)도 생긴다. 즉 "사고 싶다"(`requested`)와 "샀다"(`ordered`)는 다른 상태다. 헤더 상태·헤더 ETA 는 라인에서 **파생**된다.
- **입고 계획**: 그 발주로 어느 창고에 무엇을 얼마나 받을 예정인가. **한 발주에 계획은 하나뿐이다.** 해외 발주도 `planType='source'` 계획 1개만 만든다 — 예전에 source/destination 두 개를 만들다 입고예정 2배 + 목적지 재고 창조라는 이중계상 사고가 났다.
- **이동 지시서**: 출발 창고 → 도착 창고 선적. `draft → shipped → partially_received → closed`. **발주와 링크하지 않는다** — 실무가 "중국 창고에서 충분히 모은 뒤 선적" 이라 발주 배치와 선적 배치가 1:1 이 아니고, N:M 을 FK 로 이으면 이중계상 계열로 되돌아간다. `transfer_orders.purchase_order_id` 는 **빠진 FK 가 아니라 없는 것이 설계**다.
- **물건이 지금 어디 있는가**에 답하는 것은 세 문서의 연쇄가 아니라 **재고원장(`stock_ledgers`)** 이다. grain 은 SKU × 창고 × 로케이션 × 상태이고, 운송 중은 문서 상태가 아니라 `IN_TRANSFER` 원장 상태다.
- 판매 창고 관점의 공급 파이프라인(①발주 잔량 ②이동 대기 ③이동 중)은 문서를 조인해서가 아니라 단계별로 따로 질의해 재구성한다. **②(중국 도착·미선적)를 빼면 "재고 0, 입고예정 0" 으로 보여 중복 발주가 난다** — 그 물량은 판매가능수량에도, 입고예정에도, 발주 잔량에도 없다.
- _Avoid_: 발주 `received` 를 최종 목적지 도착으로 읽기, 한 발주에 입고 계획을 둘 만들기, 발주와 이동 지시서를 FK 로 잇기, 재고 위치를 문서 조인으로 알아내려 하기.
...
- **(구현됨 2026-07 — ADR-0027, 상황판 `docs/outbound-shipment-ledger-rfc.md`)** 출고주문은 "박스"가 아니라 **판매주문을 상품매칭 규칙으로 재고상품(SKU)으로 변환한 스냅샷**이다. 판매주문 ↔ 출고주문은 **0..1 : 0..1** (디지털-only 주문은 출고주문 없음, 보상출고 등 단독 출고주문은 판매주문 없음). 물리 박스 분할(송장분할)·합배송은 출고주문이 아니라 **상자(shipment) 레이어**에서 일어난다. 기존의 "출고주문을 2개로 쪼개는" FO 분할(`split()` / `handleFulfillmentOrderSplit`)은 은퇴 완료(코드 제거 확인, Phase 3 클러스터 B).
...
### 피킹 계획 (Picking Plan)
- 정의: 한 **출고작업(Outbound Batch)** 에 대해 "어느 상자의 어느 라인을, 어느 로케이션에서, 몇 개 집을지" 를 확정한 스냅샷. 구현 엔티티는 `picking_plans`(헤더) + `picking_plan_members`(편입된 상자) + `picking_source_allocations`(상자 라인별 출발 로케이션·수량).
- **피킹 계획은 작업 방식과 무관하다.** 개별피킹이든 토탈피킹이든 계획은 **같은 절차**로 수립되고(`plan`), 같은 절차로 시작되며(`start`), 같은 조건에서 무효화된다(재고 이동·상자 편입 변경 등 → `planStalenessReason`). 방식이 갈라지는 지점은 계획이 아니라 **그 계획을 집는 방법(`scan`)과 보관 이동(custody)** 이다.
- 따라서 계획 층은 전략 어댑터의 소유물이 아니라 그 바깥의 단일 모듈이며, 전략 이름은 계획 row 에 기록되는 **값**(`picking_plans.strategy`)일 뿐 계획 절차의 분기 조건이 아니다. 자세한 결정과 경계 규칙은 ADR-0030.
- 계획 수립은 재고 예약과 다르다. 예약은 상자 라인에 대한 **가용 재고 점유**(`stock_reservations`)이고, 피킹 계획은 이미 예약이 끝나 출고예정(`planned`)이 된 상자들에 대한 **집을 위치의 확정**이다.
- _Avoid_: 피킹 계획을 방식별로 다른 것으로 보기, 계획 수립을 재고예약이라고 부르기, `picking_plans.strategy` 를 계획 절차의 분기 조건으로 읽기.
