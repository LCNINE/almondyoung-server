...
### 재고 현황 목록
- 정의: 운영자가 SKU 단위로 현재 재고 상태를 훑어보고, 필요하면 창고 단위 세부 재고를 확인하는 목록.
- 재고 현황 목록의 기본 행 단위는 **SKU** 다. 창고 필터가 있으면 해당 창고 기준 재고를 보여주고, 창고 필터가 없으면 모든 창고를 합산한 SKU 재고를 보여준다.
- 창고 필터가 없는 합산 수량은 창고별 관점 수량의 합이다. 창고 간 이동 중인 재고는 출발 창고의 이동 대기와 도착 창고의 입고 예정에 동시에 나타날 수 있으므로, 목록의 합산 수량은 빠른 스캔용이고 창고별 상세가 최종 해석 기준이다.
- 예정 수량은 **입고 예정**, **이동 대기**, **출고 예정**으로 구분한다. 창고 간 이동은 이동 대기로 표시하고, 출고 예정은 고객 주문·출고 작업 때문에 창고에서 빠질 예정인 수량에만 쓴다.
- **SKU Group** 이 있는 SKU 는 같은 그룹의 SKU 끼리 연속해서 표시할 수 있으며, 그룹 자체를 재고 행의 기준 단위로 보지는 않는다.
- 페이지 크기는 SKU 수를 기준으로 하므로 큰 **SKU Group** 은 페이지 경계에서 나뉠 수 있다. 그룹명 병합 표시는 현재 페이지 안에서 연속된 SKU 행에만 적용한다.
- _Avoid_: 재고 현황 목록의 페이지네이션을 SKU×창고 조합 기준으로 설명하기, SKU Group 을 판매상품 variant 처럼 강결합 단위로 취급하기.
