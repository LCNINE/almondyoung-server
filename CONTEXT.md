...
### 발주 (Purchase Order) · 입고예정 (Expected Arrival) · 이동 지시서 (Transfer Order) *(결정됨 — ADR-0039)*
- **발주**: 공급사에게 물건을 사는 문서. 소유 구간은 **공급사 → 출발 창고 도착**이고 거기서 `received` 로 종결한다. 해외 발주에서 출발 창고는 중국 물류창고이므로 **발주의 `received` 는 "중국 창고 입고 완료" 를 뜻한다** — 최종 목적지(부천) 도착이 아니다. `received` 는 **「더 받을 것이 없다」**(전 라인이 전량 입고 또는 잔량 포기)이다.
...
- _Avoid_: 발주 `received` 를 최종 목적지 도착으로 읽기, 입고예정을 발주 밖의 문서로 다시 만들기, 발주와 이동 지시서를 FK 로 잇기, 재고 위치를 문서 조인으로 알아내려 하기.
