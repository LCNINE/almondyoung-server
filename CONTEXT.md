...
- 작업 방식 두 가지. **개별피킹**(현행): 출고주문을 하나씩 — 송장 한 장 집어 바구니에 담고 포장대에서 상품 바코드로 검수·포장 후 송장 바코드 스캔으로 출고 (※ 재설계 후엔 송장 스캔 = 박스 open, 출고 종결은 검수 자동완료 — `상자(Shipment)·운송장번호` 항목 참조). **토탈피킹**(*도입 예정, 미구현*): 번호 붙은 바구니가 달린 피킹 카트로 출고작업 하나(=바구니 수만큼의 출고주문)를 한 번에 처리 — 단말기가 SKU 단위로 병합한 리스트와 로케이션을 지시하고, 작업자가 한 SKU 를 여러 바구니(=출고주문)에 분배한다.
