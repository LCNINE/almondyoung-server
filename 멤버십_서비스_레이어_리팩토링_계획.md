...
- `SubscriptionCancellationService` → Reader + Manager 분리 완료

### 🔄 리팩토링 필요 서비스

1. **EntitlementService** (가장 복잡)
2. **SubscriptionService** (핵심 서비스)
3. **PauseService**
4. **PlanService**
5. **BenefitTrackingService**
6. **RecurringBillingService**
...
└── pause.manager.ts         # 일시정지 생성/취소
```

#### 🔧 pause.service.ts (리팩토링 후)
...
export class PauseService {
  ...
  async pauseSubscription(userId, pauseDays, reason) {
    ...
    if (!entitlement) throw new Error('No active subscription');

    return this.pauseManager.createPause(
      userId,
      entitlement,
      pauseDays,
      reason,
  ...
  async cancelPause(userId, reason) {
    ...
    return this.pauseManager.cancelPause(userId, entitlement, reason);
  }
}
```
