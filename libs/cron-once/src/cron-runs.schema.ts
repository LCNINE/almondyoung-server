 ...
 *
 * 이 파일은 크론이 있는 앱 8개(analytics · channel-adapter · core · membership · notification ·
 * ugc-service · user-service · wallet)의 `drizzle.config.ts` 가 **전부 schema 목록에 물고 있다**. 여기를 고치면
 * 그 8개 앱 모두에 마이그레이션이 생긴다 — 하나라도 빠뜨리면 다음 무관한 `db:generate` 가 이 변경을
