 ...
 * 판정:
 *   - 행 수 ≥ 2 (최소 두 주기가 지났다)
 *   - 두 컨텍스트 실행 카운터 합 = 행 수 (주기마다 정확히 한 번) — exactly-once 는 이 등식이 지킨다
 *   - claimed_by 는 항상 'a' 또는 'b' 다
 * 한 프로세스라 시계는 공유하지만 선점은 DB 에서 일어나므로 경쟁 자체는 실물이다.
 * «양쪽이 모두 이기는 것» 은 요구사항이 아니다 — 한 프로세스에서 a 의 타이머가 먼저 만들어져
 * 대체로 a 가 먼저 쏘는 편향이 있고, 표본이 몇 주기뿐이면 b 가 한 번도 못 이길 수 있다.
 */
...
const buildApp = async (instanceId: 'a' | 'b'): Promise<TestingModule> => {
  ...
  @Module({
    imports: [
      SCHEDULE_ROOT,
      CronOnceModule,
      DbModule.forRoot({ config: { connectionString: DATABASE_URL as string }, schema: {} }),
    ],
...
describeIfDb('cron-once 선점 경쟁 (앱 컨텍스트 둘)', () => {
  ...
  it('주기마다 정확히 한 인스턴스만 실행한다', async () => {
    ...
    // afterAll 에서 한 번만 한다.
    void a.get(SchedulerRegistry).getCronJob(PROBE).stop();
    void b.get(SchedulerRegistry).getCronJob(PROBE).stop();
    ...
    // exactly-once 의 증거: 두 인스턴스가 각자 센 실행 횟수의 합이 선점된 행 수와 같아야 한다.
    expect(counters.a + counters.b).toBe(rows.length);
    // 「양쪽이 모두 이긴다」는 요구사항이 아니다(같은 프로세스의 타이머 순서 편향으로 한쪽이
    // 몇 주기 안에서 계속 이길 수 있다) — claimed_by 가 둘 중 하나이기만 하면 된다.
    for (const row of rows) {
      expect(['a', 'b']).toContain(row.claimed_by);
    }
