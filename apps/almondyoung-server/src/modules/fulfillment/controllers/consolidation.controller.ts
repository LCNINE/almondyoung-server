...
export class ConsolidationController {
      ...
      rules: [
        {
          ...
          criteria: { addressMatch: 'exact', customerMatch: true, serviceMatch: true, timeWindow: 24 },
          constraints: { maxOrdersPerGroup: 5, maxTotalWeight: 30, maxTotalValue: 1000000 },
        },
        {
          ...
          criteria: { addressMatch: 'exact', customerMatch: false, serviceMatch: true, timeWindow: 12 },
          constraints: { maxOrdersPerGroup: 3, maxTotalWeight: 20, maxTotalValue: 500000 },
        },
        {
          ...
          priority: 3,
          autoConsolidate: false,
          criteria: { addressMatch: 'fuzzy', customerMatch: true, serviceMatch: true, timeWindow: 48, maxDistance: 5 },
          constraints: { maxOrdersPerGroup: 3, maxTotalWeight: 25, maxTotalValue: 800000 },
        },
      ],
    };
  }
    ...
    return {
      ...
      opportunities: {
        immediate: {
          ...
          groups: autoConsolidateGroups.slice(0, 5),
        },
        reviewRequired: {
          ...
          groups: manualReviewGroups.slice(0, 5),
        },
      },
  ...
  async getSavingsProjection(@Param('warehouseId') warehouseId: string, @Query('days') days: number = 30) {
    ...
    return {
      ...
      projection: {
        ...
        carbonFootprintReduction: Math.round(groups.length * 0.5),
      },
      breakdown: {
  ...
  private generateRecommendations(groups: ConsolidationGroup[]): string[] {
    ...
    if (autoGroups > 0) recommendations.push(`${autoGroups}개의 주문 그룹을 즉시 자동 합배송할 수 있습니다`);
    if (manualGroups > 0) recommendations.push(`${manualGroups}개의 주문 그룹이 수동 검토를 통한 합배송 가능합니다`);
    if (groups.length === 0) recommendations.push('현재 합배송 가능한 주문이 없습니다');
    ...
    if (highValueGroups > 0) recommendations.push(`고효율 합배송 기회 ${highValueGroups}건이 발견되었습니다`);
