// CMS 자동이체 payerNumber(개인 생년월일 6자리 또는 사업자번호 10자리) 형식 검증.
// 효성 D+1 실명대사에서 Q201(생년월일/사업자번호 불일치)로 실패하기 전에, 제출 단계에서
// 잡을 수 있는 오타성 오류(사업자번호 체크섬 불일치·생년월일 범위 초과)를 사전 차단한다.
// wallet 백엔드(apps/wallet/src/cms/payer-number.ts)와 동일 규칙.
...
export function isValidPayerNumber(value: string): boolean {
  ...
  if (/^\d{10}$/.test(normalized)) return isValidBusinessRegistrationNumber(normalized);
  return false;
}
