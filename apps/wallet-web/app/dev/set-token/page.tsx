...
export default function SetTokenPage() {
  ...
  function handleSet() {
    ...
    // access_token 쿠키 설정 (100년 = 3153600000초)
    document.cookie = `access_token=${token.trim()}; path=/; max-age=3153600000; SameSite=Lax`;
    setMessage('✅ access_token 쿠키가 설정됐습니다.');
  }

  function handleClear() {
    document.cookie = 'access_token=; path=/; max-age=0';
    setMessage('🗑 access_token 쿠키가 삭제됐습니다.');
    setToken('');
  }

  return (
    <div className="max-w-xl mx-auto mt-16 p-6 space-y-4 border rounded-lg">
      <h1 className="text-lg font-semibold">Dev: access_token 설정</h1>
      <p className="text-sm text-muted-foreground">
        테스트용 JWT를 붙여넣으면 <code>access_token</code> 쿠키로 저장됩니다.
      </p>
      <textarea
