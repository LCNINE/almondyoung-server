...
export const policyRuleType = pgEnum('policy_rule_type', [
  'MAX_PAUSES_PER_YEAR',
...
export const subscriptionChangeType = pgEnum('subscription_change_type', [
  'UPGRADE',
  'DOWNGRADE',
  'RENEWAL',
  'INITIAL',
...
export const subscriptionEntitlement = pgTable(
...
export const membershipDunningQueue = pgTable(
  'membership_dunning_queue',
  {
    id: uuid().defaultRandom().primaryKey().notNull(),
...
export const membershipDiscountEvents = pgTable(
  ...
  (table) => [
    ...
    index('idx_events_user_cycle').using(
      'btree',
      table.userId.asc().nullsLast().op('date_ops'),
      table.cycleStartDate.asc().nullsLast().op('date_ops'),
    ),
  ],
);
...
export const membershipCycleBenefits = pgTable(
  ...
  (table) => [
    ...
    primaryKey({
      columns: [table.userId, table.cycleStartDate],
      name: 'membership_cycle_benefits_user_id_cycle_start_date_pk',
    }),
  ],
);
