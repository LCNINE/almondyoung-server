...
export class BillingController {
  ...
  async getUserPaymentProfile(@Param('userId') userId: string) {
    try {
      const profile = await this.paymentClientService.getDefaultPaymentProfile(userId);
      return {
  ...
  async getPaymentAttempt(@Param('attemptId') attemptId: string) {
    try {
      const attempt = await this.paymentClientService.getPaymentAttempt(attemptId);
      return {
  ...
  async getHealthStatus() {
    ...
    try {
      // 임시 사용자로 연결 테스트
      await this.paymentClientService.getDefaultPaymentProfile('health-check-user');
      return {
        status: 'healthy',
