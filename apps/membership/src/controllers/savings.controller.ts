...
export class SavingsController {
  ...
  async getCurrentMonthSavings(@User('userId') userId: string): Promise<MonthlySavingsDto> {
  ...
  async getMonthSavings(
    ...
    if (!/^\d{4}-\d{2}$/.test(yearMonth)) {
      throw new BadRequestException('Invalid yearMonth format. Use YYYY-MM (e.g., 2025-11)');
    }
  ...
  async getRangeSavings(
    ...
    if (isNaN(startDate.getTime()) || isNaN(endDate.getTime())) {
      throw new BadRequestException('Invalid date format. Use ISO 8601 format (e.g., 2025-09-01)');
    }
