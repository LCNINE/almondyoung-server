...
export class PolicyManagementService {
   ...
   */
  async getAllPolicies(query?: GetPoliciesDto): Promise<PolicyListResponse> {
    try {
      const {
        ruleType,
        tierId,
      ...
      if (ruleType) {
        conditions.push(eq(schema.subscriptionPolicies.ruleType, ruleType as any));
      }
   ...
   */
  async createPolicy(createPolicyDto: CreatePolicyDto): Promise<PolicyResponse | null> {
    try {
      // 입력 검증
   ...
   * 정책을 비활성화합니다.
   */
  async deactivatePolicy(policyId: string): Promise<{ success: boolean; message: string }> {
    try {
      // 기존 정책 존재 확인
      const existingPolicy = await this.getPolicyById(policyId);
      if (!existingPolicy) {
   ...
   * 정책 입력값을 검증합니다.
   */
  private async validatePolicyInput(input:   CreatePolicyDto): Promise<void> {
    // 티어 ID 검증
    if (input.tierId) {
      const tier = await this.dbService.db
        .select()
    ...
    if (input.validFrom && input.validUntil) {
      ...
      if (fromDate >= untilDate) {
        throw new BadRequestException('validFrom must be earlier than validUntil');
      }
    }
   ...
   * 정책 규칙 값을 검증합니다.
   */
  private validatePolicyRuleValue(ruleType: string, ruleValue: Record<string, any>): void {
    switch (ruleType) {
      case 'MAX_PAUSES_PER_YEAR':
        if (!ruleValue.maxPauses || typeof ruleValue.maxPauses !== 'number' || ruleValue.maxPauses < 0) {
          throw new BadRequestException('MAX_PAUSES_PER_YEAR requires positive maxPauses number');
        }
        break;

      case 'MIN_PAUSE_DURATION_DAYS':
        if (!ruleValue.minDays || typeof ruleValue.minDays !== 'number' || ruleValue.minDays < 1) {
          throw new BadRequestException('MIN_PAUSE_DURATION_DAYS requires positive minDays number');
        }
        break;

      case 'MAX_PAUSE_DURATION_DAYS':
        if (!ruleValue.maxDays || typeof ruleValue.maxDays !== 'number' || ruleValue.maxDays < 1) {
          throw new BadRequestException('MAX_PAUSE_DURATION_DAYS requires positive maxDays number');
        }
        break;

      case 'PAUSE_COOLDOWN_DAYS':
        if (!ruleValue.cooldownDays || typeof ruleValue.cooldownDays !== 'number' || ruleValue.cooldownDays < 0) {
          throw new BadRequestException('PAUSE_COOLDOWN_DAYS requires non-negative cooldownDays number');
        }
        break;
   ...
   * 중복 정책을 확인합니다.
   */
  private async checkDuplicatePolicy(input: CreatePolicyDto): Promise<void> {
  ...
  async createPolicyVersion(
    policyId: string, 
    changes: { ruleValue: Record<string, any>; changeReason?: string; changedBy?: string }
  ): Promise<PolicyResponse | null> {
    try {
      // 기존 정책 확인
      const existingPolicy = await this.getPolicyById(policyId);
      ...
      // 변경사항이 있는지 확인
      const hasChanges = JSON.stringify(existingPolicy.ruleValue) !== JSON.stringify(changes.ruleValue);
      if (!hasChanges) {
        throw new BadRequestException('No changes detected in policy rule value');
      }
      ...
      const newPolicy = await this.dbService.db
        ...
        .from(schema.subscriptionPolicies)
        .where(and(
          eq(schema.subscriptionPolicies.ruleType, existingPolicy.ruleType as any),
      ...
      this.logger.error(`Failed to create policy version for ${policyId}:`, error);
      throw error;
    }
  }
  ...
  async getPolicyVersions(policyId: string): Promise<PolicyVersion[]> {
    try {
      ...
      const allVersions = await this.dbService.db
        ...
        .from(schema.subscriptionPolicies)
        .where(and(
          eq(schema.subscriptionPolicies.ruleType, basePolicy.ruleType as any),
          basePolicy.tierId 
      ...
      this.logger.error(`Failed to get policy versions for ${policyId}:`, error);
      throw error;
    }
  }
   ...
   * 정책을 특정 버전으로 롤백합니다.
   */
  async rollbackToVersion(policyId: string, targetVersion: number): Promise<PolicyResponse | null> {
    try {
      // 모든 버전 조회
      ...
      if (targetVersionData.isActive) {
        throw new BadRequestException(`Version ${targetVersion} is already active`);
      }
      ...
      await this.dbService.db.transaction(async (tx) => {
        ...
        await tx
          ...
          .set({ isActive: false, updatedAt: new Date() })
          .where(and(
            eq(schema.subscriptionPolicies.ruleType, currentPolicy.ruleType as any),
      ...
      this.logger.log(`Policy ${policyId} rolled back to version ${targetVersion}`);

      return this.getPolicyById(targetVersionData.id);
    } catch (error) {
      this.logger.error(`Failed to rollback policy ${policyId} to version ${targetVersion}:`, error);
      throw error;
    }
  }
  ...
  async comparePolicyVersions(
    ...
    try {
      ...
      // 차이점 분석
      const differences = this.findPolicyDifferences(v1.ruleValue, v2.ruleValue);
   ...
   * 정책 변경 이력을 조회합니다.
   */
  async getPolicyChangeHistory(policyId: string): Promise<Array<{
    version: number;
    changedAt: Date;
    ...
    try {
      ...
      this.logger.error(`Failed to get policy change history for ${policyId}:`, error);
      throw error;
    }
  }
}