...
export class InvoiceSummaryDto {
  ...
  @ApiProperty({
    description: 'Invoice 상태',
    enum: ['issued', 'printed', 'shipped', 'canceled'],
  })
  status: string;
  ...
  @ApiProperty({
    description: '송장 발급 방식',
    enum: ['goodsflow', 'direct', 'self'],
  })
  issueMethod: string;
}
...
export class FulfillmentOrderResponseDto {
  ...
  @ApiProperty({
    description: 'Invoice 정보 (Invoice가 없으면 null)',
