...
export class ReturnService {
  ...
  async createReturnRequest(params: {
    ...
    return this.inTx(async (trx) => {
      ...
      const returnItems = await trx
        .insert(wmsTables.returnItems)
  ...
  async inspectReturn(params: {
    ...
    return this.inTx(async (trx) => {
      ...
      // 반품 헤더 업데이트
      const allQcCompleted = params.items.every(
        item => item.qcStatus === 'passed' || item.qcStatus === 'failed'
      );

      const overallStatus = params.items.some(item => item.qcStatus === 'passed')
        ? 'qc_passed'
        : 'qc_failed';
      ...
      for (const item of params.items) {
        ...
        if (item.action === 'restock') {
          ...
          if (!item.targetLocationId) {
            throw new BadRequestException(
              `Target location is required for restocking item ${item.returnItemId}`
            );
          }

          if (!returnItem.locationId) {
            throw new BadRequestException(
              `Return item ${item.returnItemId} has no current location`
            );
          }
          ...
          this.logger.log(
            `Disposed ${item.quantity} units of SKU ${returnItem.skuId}`
          );
        }
      }
