...
export class InventoryCommandService {
  ...
  async receive(input: {
    ...
    const exec = async (trx: DbTx) => {
      ...
      this.logger.log(
        `RECEIVE: sku=${sku.name} qty=${input.quantity} (${currentQuantity} → ${afterQuantity})`
      );
  ...
  async ship(input: {
    ...
    const exec = async (trx: DbTx) => {
      ...
      this.logger.log(
        `SHIP: sku=${sku.name} qty=${input.quantity} (${currentQuantity} → ${afterQuantity})`
      );
  ...
  async adjustUp(input: {
    ...
    const exec = async (trx: DbTx) => {
      ...
      this.logger.log(
        `ADJUST_UP: sku=${sku.name} qty=${input.quantity} (${currentQuantity} → ${afterQuantity})`
      );
  ...
  async adjustDown(input: {
    ...
    const exec = async (trx: DbTx) => {
      ...
      this.logger.log(
        `ADJUST_DOWN: sku=${sku.name} qty=${input.quantity} (${currentQuantity} → ${afterQuantity})`
      );
