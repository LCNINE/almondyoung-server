...
export class LocationService {
    ...
    await this.inTx(async (trx) => {
      for (const role of roles) {
        ...
        if (!exists) {
          ...
          this.logger.log(
            `System location created for ${warehouseId}: ${role}`,
          );
        }
      }
    }, tx);
  ...
  async getSystemLocationByRole(
    warehouseId: string,
    role: SystemLocationRole,
    tx?: DbTx,
  ) {
    if (tx) {
      ...
      if (!loc) {
        throw new NotFoundException(
          `System location not found for role ${role} in warehouse ${warehouseId}`,
        );
      }
      return loc;
    } else {
  ...
  async createColumn(warehouseId: string, dto: CreateColumnDto) {
    this.logger.log(
      `Creating column ${dto.columnName} for warehouse ${warehouseId}`,
    );
    ...
    if (existing.length > 0) {
      throw new BadRequestException(
        `Column ${dto.columnName} already exists in this warehouse`,
      );
    }
  ...
  async getColumns(warehouseId: string, isActive?: boolean) {
    ...
    return this.db
      ...
      .orderBy(
        asc(wmsTables.locationColumns.displayOrder),
  ...
  async createRack(
    ...
    this.logger.log(
      `Creating rack ${dto.columnName}-${dto.rackNumber} for warehouse ${warehouseId}`,
    );
    ...
    return await this.db.transaction(async (tx) => {
      ...
      if (!column) {
        throw new BadRequestException(
          `Column ${dto.columnName} not found in warehouse`,
        );
      }
      ...
      if (existing.length > 0) {
        throw new BadRequestException(
          `Rack ${dto.columnName}-${dto.rackNumber} already exists`,
        );
      }
      ...
      if (dto.binSettings.customBins && dto.binSettings.customBins.length > 0) {
        const customLocations: Array<typeof wmsTables.locations.$inferInsert> =
          dto.binSettings.customBins.map((customBinName) => {
      ...
      this.logger.log(
        `Created rack ${dto.columnName}-${dto.rackNumber} with ${createdLocationCodes.length} bins`,
      );
  ...
  async createZoneLocation(warehouseId: string, dto: CreateZoneLocationDto) {
    this.logger.log(
      `Creating zone location ${dto.code} for warehouse ${warehouseId}`,
    );
    ...
    return await this.db.transaction(async (tx) => {
      ...
      if (/[ㄱ-ㅎ|ㅏ-ㅣ|가-힣]/.test(dto.code)) {
        ...
        this.logger.log(
          `Korean characters detected in "${dto.code}", generated code: ${locationCode}`,
        );
      }
      ...
      this.logger.log(
        `Created zone location: ${location.displayName} (code: ${location.code})`,
      );
      return location;
    });
  }
  ...
  async getLocations(warehouseId: string, query: LocationQueryDto) {
    ...
    if (query.columnName) {
      conditions.push(
        eq(wmsTables.locationColumns.columnName, query.columnName),
      );
    }
    if (query.rackNumber) {
  ...
  async addCustomBin(warehouseId: string, dto: AddCustomBinDto) {
    this.logger.log(
      `Adding custom bin ${dto.customBinName} to rack ${dto.columnName}-${dto.rackNumber}`,
    );
    ...
    return await this.db.transaction(async (tx) => {
      ...
      if (!rackResult) {
        throw new BadRequestException(
          `Rack ${dto.columnName}-${dto.rackNumber} not found`,
        );
      }
      ...
      if (existing.length > 0) {
        throw new BadRequestException(
          `Location ${locationCode} already exists`,
        );
      }
