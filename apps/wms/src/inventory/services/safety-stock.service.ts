...
import { wmsTables, wmsSchema, wmsViews, DbTx } from '../../../database/schemas/wms-schema';
...
export class SafetyStockService {
    ...
    async getBelowSafetyStock(warehouseId?: string, tx?: DbTx): Promise<SafetyStockWarning[]> {
        return this.inTx(async (tx) => {
            ...
            const conditions = [
                sql`COALESCE(${wmsViews.stockSummary.onHandQty}, 0) < ${wmsTables.skus.safetyStock}`
            ];
            ...
            const query = tx
                .select({
                    ...
                    currentStock: sql<number>`COALESCE(${wmsViews.stockSummary.onHandQty}, 0)`,
