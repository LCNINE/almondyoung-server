...
export class PurchaseOrderService {
  ...
  async createPurchaseOrder(
    ...
    return this.inTx(async (trx) => {
      ...
      const sourceWarehouseId = await this.getSupplierDefaultWarehouseId(
      ...
      const [purchaseOrder] = await trx
        ...
        .values({
          ...
          expectedArrival: createDto.expectedArrival
            ? new Date(createDto.expectedArrival)
            : null,
          status: 'created',
      ...
      this.logger.log(
        `Created purchase order ${purchaseOrder.id} with ${purchaseOrderLines.length} lines`,
      );
    ...
    return this.inTx(async (trx) => {
      ...
      if (types.length > 1) {
        throw new BadRequestException(
          'All cart items must have the same purchase order type',
        );
      }
      ...
      this.logger.log(
        `Created purchase order ${purchaseOrder.id} from ${cartItems.length} cart items`,
      );
    ...
    return this.inTx(async (trx) => {
      ...
      await trx
        ...
        .set({
          ...
          expectedArrival: updateDto.expectedArrival
            ? new Date(updateDto.expectedArrival)
            : undefined,
          updatedAt: new Date(),
        })
        .where(eq(wmsTables.purchaseOrders.id, poId));
      ...
      this.logger.log(
        `Updated purchase order ${poId} status to ${updateDto.status}`,
      );
  ...
  private async createInboundPlanFromPO(tx: DbTx, poId: string): Promise<void> {
    ...
    if (requiresTransfer) {
      ...
      await tx
        ...
        .values([...sourceItems, ...destinationItems]);
      ...
      this.logger.log(
        `Created single inbound plan ${plan.id} for domestic PO ${poId}`,
      );
    }
  }
   ...
   */
  private async getSupplierDefaultWarehouseId(
    supplierId: string,
    tx: DbTx,
  ): Promise<string> {
    ...
    return this.inTx(async (trx) => {
      ...
      const lines = await trx
        .select({
          ...
          skuBarcode: wmsTables.skus.defaultBarcode,
        })
        .from(wmsTables.purchaseOrderLines)
        .leftJoin(
          wmsTables.skus,
          eq(wmsTables.purchaseOrderLines.skuId, wmsTables.skus.id),
        )
        .where(eq(wmsTables.purchaseOrderLines.poId, poId));
        ...
        ? (() =>
            trx
              .select({
                ...
                contactInfo: wmsTables.suppliers.contactInfo,
              })
              .from(wmsTables.suppliers)
    ...
    const cartItems = await this.inTx(
      async (trx) =>
        trx
          ...
          .leftJoin(
            wmsTables.skus,
            eq(wmsTables.purchaseOrderCart.skuId, wmsTables.skus.id),
          )
  ...
  async clearCart(type?: PurchaseOrderType, tx?: DbTx): Promise<void> {
    if (type) {
      ...
      await this.inTx(
        async (trx) => trx.delete(wmsTables.purchaseOrderCart),
        tx,
      );
    }
