...
export class InboundService {
  ...
  // 입고 라인 메모 수정
  async updateInboundLineMemo(
    lineId: string,
    dto: UpdateInboundLineMemoDto,
    tx?: DbTx,
  ) {
  ...
  async simpleInbound(dto: SimpleInboundDto, tx?: DbTx) {
    ...
    return this.inTx(async (tx) => {
      ...
      const inboundZone = await this.locationService.getSystemLocationByRole(
        warehouseId,
        'inbound_default',
        tx,
      );
      if (!inboundZone)
        throw new BadRequestException('입고 기본존이 존재하지 않습니다.');
  ...
  async getInboundPending(
    ...
    return this.inTx(async (tx) => {
      ...
      return {
        ...
        totalPendingQuantity: inboundPending.reduce(
          (sum, plan) => sum + plan.totalPendingQuantity,
          0,
        ),
      ...
      .where(
        and(
          ...
          warehouseId
            ...
            : undefined,
          method
            ? eq(wmsTables.inboundReceipts.method as any, method as any)
            : undefined,
          skuId ? eq(wmsTables.inboundReceiptLines.skuId, skuId) : undefined,
          startDate
            ? gte(wmsTables.inboundReceipts.occurredAt, new Date(startDate))
            : undefined,
          endDate
            ? lte(
  ...
  async listInboundWorkLogs(
    ...
    const logs = await this.db
      ...
      .where(
        and(
          warehouseId
            ? eq(wmsTables.inboundWorkLogs.warehouseId, warehouseId)
            : undefined,
          skuId ? eq(wmsTables.inboundWorkLogs.skuId, skuId) : undefined,
          type
            ? eq(wmsTables.inboundWorkLogs.type as any, type as any)
            : undefined,
          method
            ? eq(wmsTables.inboundWorkLogs.method as any, method as any)
            : undefined,
          startDate
            ? gte(wmsTables.inboundWorkLogs.timestamp, new Date(startDate))
            : undefined,
          endDate
            ? lte(
                wmsTables.inboundWorkLogs.timestamp,
                new Date(new Date(endDate).setHours(23, 59, 59, 999)),
              )
            : undefined,
        ),
    ...
    const items = rows
      .map((r) => {
        const confirmed = Math.max(
          0,
          (r.qtyReceived ?? 0) - (r.qtyReturned ?? 0),
        );
  ...
  async addInboundPlanItems(dto: AddInboundPlanItemsDto, tx?: DbTx) {
    return this.inTx(async (tx) => {
      const plan = await tx.query.inboundPlans.findFirst({
        where: eq(wmsTables.inboundPlans.id, dto.planId),
      });
    ...
    const rows = await this.db
      ...
      .where(
        and(
          ...
          startDate
            ? gte(wmsTables.inboundPlans.expectedDate, new Date(startDate))
            : undefined,
          endDate
            ? lte(
                wmsTables.inboundPlans.expectedDate,
                new Date(new Date(endDate).setHours(23, 59, 59, 999)),
              )
            : undefined,
        ),
  ...
  async receiveFromPlan(dto: ReceiveFromPlanDto, tx?: DbTx) {
    return this.inTx(async (tx) => {
      ...
      const newStatus =
        newReceived >= item.expectedQty ? 'confirmed' : 'pending';
      await tx
  ...
  async putawayFromOrigin(dto: PutawayRequestDto, tx?: DbTx) {
    return this.inTx(async (tx) => {
      ...
      if (!originLocationId)
        throw new BadRequestException('origin location missing');
      ...
      if (!dest.isActive)
        throw new BadRequestException('destination location is inactive');
  ...
  async cancelInbound(dto: CancelInboundDto, tx?: DbTx) {
    return this.inTx(async (tx) => {
      ...
      if ((line.returnedQty ?? 0) > 0) {
        throw new BadRequestException(
          'cannot cancel: returns exist; cancel returns first',
        );
      }
      ...
      if (onHand < line.quantity) {
        throw new BadRequestException(
          'insufficient on-hand at origin to cancel',
        );
      }
      ...
      if (!line.eventId) {
        throw new BadRequestException(
          'original receive eventId missing; cannot perform reversal',
        );
      }
      const rev = await this.eventStore.reverseEvent(
        line.eventId,
        'CANCEL',
        tx,
      );
      ...
      const allCanceled = lines.every(
        (l) => (l.canceledQty ?? 0) >= (l.quantity ?? 0),
      );
      if (allCanceled) {
        await tx
          .update(wmsTables.inboundReceipts)
  ...
  async getInboundHistory(
    ...
    return {
      ...
      totalInboundQuantity: inboundEvents.reduce(
    ...
    if (!skuBarcode) {
      ...
      if (!sku) {
        throw new NotFoundException(
          `바코드 ${barcode}에 해당하는 SKU를 찾을 수 없습니다.`,
        );
      }
