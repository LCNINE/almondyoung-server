...
export class InboundController {
  ...
  @ApiOperation({
    summary: '전수조사 간편입고 - (서버는 간편입고와 동일 처리, 기록만 구분)',
  })
  @ApiResponse({
    status: 201,
  ...
  async updateInboundLineMemo(
    ...
    @Body() dto: UpdateInboundLineMemoDto,
  ) {
  ...
  @ApiResponse({
    status: 200,
    description: '입고 예정 목록이 성공적으로 조회되었습니다.',
  })
  ...
  @ApiResponse({
    status: 200,
    description: '입고 실적이 성공적으로 조회되었습니다.',
  })
  ...
  @ApiResponse({
    status: 200,
    description: '바코드가 성공적으로 검증되었습니다.',
  })
  @ApiResponse({
    status: 404,
    description: '바코드에 해당하는 SKU를 찾을 수 없습니다.',
  })
  @ApiResponse({
    status: 400,
    description: '스캔한 SKU가 예상 SKU와 다릅니다.',
  })
  async verifyInboundByBarcode(
    @Body() dto: { barcode: string; expectedSkuId?: string },
  ) {
  ...
  @Get('receipts')
  @ApiOperation({
    summary: '입고내역(현황) 조회 - (sku, quantity, occurredAt, method)',
  })
  ...
  @Get('work-logs')
  @ApiOperation({
    summary: '입고 작업 타임라인 조회 (INBOUND/PUTAWAY/RETURN/CANCEL)',
  })
  @ApiQuery({ name: 'warehouseId', required: false })
  @ApiQuery({ name: 'skuId', required: false })
  @ApiQuery({
    name: 'type',
    required: false,
    enum: ['INBOUND', 'PUTAWAY', 'RETURN', 'CANCEL'],
  })
  @ApiQuery({
    name: 'method',
    required: false,
  ...
  @Post('putaway')
  @ApiOperation({
    summary: '입고 적치(즉시 이동): 원위치에서 목적지로 즉시 이동',
  })
  @ApiResponse({
    status: 201,
    description: '적치가 성공적으로 처리되었습니다.',
  })
  ...
  @ApiResponse({
    status: 201,
    description: '입고취소가 성공적으로 처리되었습니다.',
  })
