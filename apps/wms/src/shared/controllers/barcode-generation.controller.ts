...
export class BarcodeGenerationController {
  ...
  async generateFulfillmentOrderBarcode(
    ...
    return this.barcodeService.generateFulfillmentOrderBarcodeImage(dto.fulfillmentOrderId, dto.format ?? 'CODE128');
  }
  ...
  async validateBarcode(@Body() body: { value: string; format: string }): Promise<{
    ...
    return {
      ...
      format: body.format,
      isValid,
      message: isValid ? `Valid barcode format for ${body.format}` : `Invalid barcode value for format ${body.format}`,
    };
  }
}
