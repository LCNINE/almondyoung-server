...
export const matchingPriorityEnum = pgEnum('matching_priority', [
  'normal',
  'high',
]);
...
export const inboundReceiptStatusEnum = pgEnum('inbound_receipt_status', [
  'posted',
  'voided',
]);
export const inboundWorkTypeEnum = pgEnum('inbound_work_type', [
  'INBOUND',
  'PUTAWAY',
  'RETURN',
  'CANCEL',
...
// FOI 기반 확장 enums
export const pickingMethodEnum = pgEnum('picking_method', [
  'individual',
  'total_picking',
]);
...
export const invoiceMethodEnum = pgEnum('invoice_method', [
  'goodsflow',
  'direct',
  'self',
]);
...
export const inventoryMasterStatusEnum = pgEnum('inventory_master_status', [
  'active',
  'archived',
]);
...
export const suppliers = pgTable('suppliers', {
  ...
  defaultWarehouseId: uuid('default_warehouse_id').references(
    () => warehouses.id,
    { onDelete: 'restrict' },
  ),
...
export const movementJobs = pgTable(
  ...
  (t) => ({
    idxMovementJobsWhTime: index('idx_movement_jobs_wh_time').on(
      t.warehouseId,
      t.occurredAt,
    ),
  }),
...
export const movementJobLines = pgTable(
  'movement_job_lines',
  {
    ...
    eventId: uuid('event_id').references(() => stockEvents.id, {
      onDelete: 'set null',
    }),
    memo: varchar('memo', { length: 255 }),
    createdAt: timestamp('created_at', { withTimezone: true })
...
export const movementWorkLogs = pgTable(
  'movement_work_logs',
  {
    ...
    timestamp: timestamp('timestamp', { withTimezone: true })
      .notNull()
      .defaultNow(),
    jobId: uuid('job_id').references(() => movementJobs.id, {
      onDelete: 'set null',
    }),
    lineId: uuid('line_id').references(() => movementJobLines.id, {
      onDelete: 'set null',
    }),
...
export const skus = pgTable(
  'skus',
  {
    ...
    importDeclarationNumber: varchar('import_declaration_number', {
      length: 100,
    }),
    logisticsPartnerId: uuid('logistics_partner_id').references(
      () => suppliers.id,
      { onDelete: 'set null' },
    ),
    ...
    secondaryLocationId: uuid('secondary_location_id').references(
      () => locations.id,
      { onDelete: 'set null' },
    ),
  ...
  (t) => ({
    ...
    idxSkusPrimaryLocation: index('idx_skus_primary_location').on(
      t.primaryLocationId,
    ),
...
export const skuVariantPricing = pgTable(
  'sku_variant_pricing',
  {
    id: uuid('id').primaryKey().defaultRandom(),
    skuId: uuid('sku_id')
...
export const skuLocationMovements = pgTable(
  ...
  (t) => ({
    ...
    idxMovementTimestamp: index('idx_movement_timestamp').on(
      t.movementTimestamp,
    ),
  }),
);
...
export const skuGroups = pgTable(
  'sku_groups',
  {
    ...
    inventoryMasterId: uuid('inventory_master_id').references(
      () => inventoryProductMasters.id,
      { onDelete: 'set null' },
    ),
...
export const locationColumns = pgTable(
  ...
  (t) => ({
    ...
    idxColumnsWarehouseName: index('idx_columns_warehouse_name').on(
      t.warehouseId,
      t.columnName,
    ),
  }),
...
export const locationRacks = pgTable(
  ...
  (t) => ({
    ...
    idxRacksColumnNumber: index('idx_racks_column_number').on(
      t.columnId,
      t.rackNumber,
    ),
  }),
...
export const locations = pgTable(
  'locations',
  {
    ...
    rackId: uuid('rack_id').references(() => locationRacks.id, {
      onDelete: 'cascade',
    }),
  ...
  (t) => ({
    ...
    ckLocationsSystemZone: check(
      'ck_locations_system_zone',
      sql`( is_system = false OR location_type = 'zone' )`,
    ),
    locationsWarehouseType: index('idx_locations_warehouse_type').on(
      t.warehouseId,
      t.locationType,
    ),
    locationsRackBin: index('idx_locations_rack_bin').on(
      t.rackId,
      t.binIdentifier,
    ),
  }),
...
export const stockLedgers = pgTable(
  ...
  (t) => ({
    pk: primaryKey({
      columns: [t.skuId, t.warehouseId, t.locationId, t.stockState],
    }),
    ckNonNegative: check('ck_ledgers_non_negative', sql`${t.qty} >= 0`),
    ixLookup: index('ix_ledgers_lookup').on(
      t.skuId,
      t.warehouseId,
      t.locationId,
      t.stockState,
...
export const stockSummary = pgView('stock_summary_view', {
  ...
  // 계산된 전망
  projectedAvailableQty: integer('projected_available_qty')
    .notNull()
    .default(0),
...
export const productMatchings = pgTable(
  'product_matchings',
  {
    ...
    inventoryManagement: boolean('inventory_management')
      .notNull()
      .default(false), // true: 물리적 재고 관리, false: 디지털
    ...
    alwaysSellableZeroStock: boolean('always_sellable_zero_stock')
      .notNull()
      .default(false), // 재고 0이어도 항상 판매 가능한 상품 (직배/신상품)
...
export const salesOrderLines = pgTable('sales_order_lines', {
  ...
  salesOrderId: uuid('sales_order_id')
    ...
    .notNull(),
  variantId: uuid('variant_id').notNull(), // PIM의 Variant ID
  productMatchingId: uuid('product_matching_id').references(
    () => productMatchings.id,
    { onDelete: 'set null' },
  ), // 매칭 정보
...
export const mergeGroups = pgTable('merge_groups', {
  ...
  shippingAddressHash: varchar('shipping_address_hash', {
    length: 64,
  }).notNull(),
...
export const fulfillmentOrders = pgTable('fulfillment_orders', {
  ...
  ownerId: uuid('owner_id').references(() => holders.id, {
    onDelete: 'set null',
  }),
  status: fulfillmentStatusEnum('status').notNull().default('created'),

  // 배치 및 출고 관련 필드들
  batchId: uuid('batch_id').references(() => outboundBatches.id, {
    onDelete: 'set null',
  }),
...
export const stockReservations = pgTable(
  'stock_reservations',
  {
    ...
    fulfillmentOrderItemId: uuid('fulfillment_order_item_id').references(
      () => fulfillmentOrderItems.id,
      { onDelete: 'cascade' },
    ),
  ...
  (t) => ({
    // 인덱스 추가
    targetIdx: index('stock_reservations_target_idx').on(
      t.targetType,
      t.targetId,
    ),
    skuWarehouseIdx: index('stock_reservations_sku_warehouse_idx').on(
      t.skuId,
      t.warehouseId,
    ),
...
export const outboundTasks = pgTable('outbound_tasks', {
  ...
  warehouseId: uuid('warehouse_id')
    .references(() => warehouses.id)
    .notNull(),
  mergeGroupId: varchar('merge_group_id', { length: 64 }).references(
    () => mergeGroups.id,
    { onDelete: 'set null' },
  ), // 합배송 그룹 참조
...
export const shipments = pgTable('shipments', {
  ...
  fulfillmentOrderId: uuid('fulfillment_order_id').references(
...
export const outboxEvents = pgTable(
  ...
  (t) => ({
    idxStatusNext: index('idx_outbox_status_next').on(
      t.status,
      t.nextAttemptAt,
    ),
  }),
...
export const purchaseOrders = pgTable('purchase_orders', {
  ...
  submittedForAuditAt: timestamp('submitted_for_audit_at', {
    withTimezone: true,
  }),
...
export const stocktakingLines = pgTable(
  ...
  (t) => ({
    idxStocktakingLineSession: index('idx_stocktaking_line_session').on(
      t.sessionId,
    ),
    idxStocktakingLineSku: index('idx_stocktaking_line_sku').on(t.skuId),
    idxStocktakingLineLocation: index('idx_stocktaking_line_location').on(
      t.locationId,
    ),
  }),
);
...
export const inboundReceipts = pgTable(
  ...
  (t) => ({
    idxInboundReceiptsWhTime: index('idx_inbound_receipts_wh_time').on(
      t.warehouseId,
      t.occurredAt,
    ),
  }),
...
export const inboundPlans = pgTable(
  ...
  (t) => ({
    idxInboundPlansWhDate: index('idx_inbound_plans_wh_date').on(
      t.warehouseId,
      t.expectedDate,
    ),
    idxInboundPlansDestination: index('idx_inbound_plans_destination').on(
      t.destinationWarehouseId,
      t.expectedDate,
    ),
    // 이중 입고 계획을 위한 새 인덱스들
    idxInboundPlansWarehouseTypeStatus: index(
      'idx_inbound_plans_warehouse_type_status',
    ).on(t.warehouseId, t.planType, t.status),
    idxInboundPlansParent: index('idx_inbound_plans_parent').on(t.parentPlanId),
    idxInboundPlansPurchaseOrder: index('idx_inbound_plans_purchase_order').on(
      t.linkedPurchaseOrderId,
    ),
  }),
);
...
export const inboundReceiptLines = pgTable(
  'inbound_receipt_lines',
  {
    ...
    putawayFromOriginQty: integer('putaway_from_origin_qty')
      .notNull()
      .default(0),
    // optional link to plan item
    planItemId: uuid('plan_item_id').references(() => inboundPlanItems.id, {
      onDelete: 'set null',
    }),
    createdAt: timestamp('created_at', { withTimezone: true })
      .notNull()
...
export const inboundWorkLogs = pgTable(
  'inbound_work_logs',
  {
    ...
    timestamp: timestamp('timestamp', { withTimezone: true })
      .notNull()
      .defaultNow(),
    receiptId: uuid('receipt_id').references(() => inboundReceipts.id, {
      onDelete: 'set null',
    }),
    lineId: uuid('line_id').references(() => inboundReceiptLines.id, {
      onDelete: 'set null',
    }),
    planItemId: uuid('plan_item_id').references(() => inboundPlanItems.id, {
      onDelete: 'set null',
...
export const auditLogs = pgTable(
  ...
  (t) => ({
    ...
    idxAuditCorrelationId: index('idx_audit_correlation_id').on(
      t.correlationId,
    ),

    // 복합 인덱스
    idxAuditResourceSearch: index('idx_audit_resource_search').on(
      t.resourceType,
      t.resourceId,
    ),
    idxAuditTimeModule: index('idx_audit_time_module').on(
      t.timestamp.desc(),
      t.module,
    ),
  }),
...
export const productSkuMappings = pgTable(
  ...
  (t) => ({
    idxProductWarehouse: index('idx_product_sku_mappings_product_warehouse').on(
      t.productId,
      t.warehouseId,
    ),
    idxActiveVersion: index('idx_product_sku_mappings_active').on(
      t.productId,
      t.warehouseId,
      t.isActive,
    ),
...
export const productSkuMappingItems = pgTable(
  ...
  (t) => ({
    ...
    uqMappingVariant: index('uq_product_sku_mapping_items_mapping_variant').on(
      t.mappingId,
      t.variantId,
    ),
  }),
...
export const productSkuMappingSnapshots = pgTable(
  'product_sku_mapping_snapshots',
  {
    ...
    quantity: integer('quantity').notNull(),
    mappingId: uuid('mapping_id').references(() => productSkuMappings.id, {
      onDelete: 'restrict',
    }),
  ...
  (t) => ({
    idxProduct: index('idx_product_sku_mapping_snapshots_product').on(
      t.productId,
    ),
  }),
);
...
export const fulfillmentOrderItems = pgTable(
  ...
  (t) => ({
    idxFulfillmentOrder: index('idx_fulfillment_order_items_fo').on(
      t.fulfillmentOrderId,
    ),
...
export const outboundBatches = pgTable(
  'outbound_batches',
  {
    ...
    totalItems: integer('total_items').notNull().default(0),
    totalQty: integer('total_qty').notNull().default(0),
    scheduledPickingAt: timestamp('scheduled_picking_at', {
      withTimezone: true,
    }),
    startedAt: timestamp('started_at', { withTimezone: true }),
    canceledAt: timestamp('canceled_at', { withTimezone: true }),
  ...
  (t) => ({
    idxWarehouseStatus: index('idx_outbound_batches_warehouse_status').on(
      t.warehouseId,
      t.status,
    ),
...
export const supplierCategoryMappingsRelations = relations(
  supplierCategoryMappings,
  ({ one }) => ({
...
export const deliveryProfilesRelations = relations(
  deliveryProfiles,
  ({ many }) => ({
    skus: many(skus),
  }),
...
export const skuVariantPricingRelations = relations(
  skuVariantPricing,
  ({ one }) => ({
    sku: one(skus, {
...
export const skuLocationMovementsRelations = relations(
  skuLocationMovements,
  ({ one }) => ({
    sku: one(skus, {
...
export const inventoryProductMastersRelations = relations(
  inventoryProductMasters,
  ({ many }) => ({
    skus: many(skus),
...
export const locationColumnsRelations = relations(
  locationColumns,
  ({ one, many }) => ({
...
export const locationRacksRelations = relations(
  locationRacks,
  ({ one, many }) => ({
...
export const productMatchingsRelations = relations(
  productMatchings,
  ({ one, many }) => ({
    master: one(inventoryProductMasters, {
...
export const productVariantSkuLinksRelations = relations(
  productVariantSkuLinks,
  ({ one }) => ({
...
export const fulfillmentOrderLinesRelations = relations(
  fulfillmentOrderLines,
  ({ one }) => ({
...
export const fulfillmentOrderItemsRelations = relations(
  fulfillmentOrderItems,
  ({ one, many }) => ({
    fulfillmentOrder: one(fulfillmentOrders, {
...
// Outbound Relations
export const outboundTasksRelations = relations(
  outboundTasks,
  ({ one, many }) => ({
    warehouse: one(warehouses, {
...
export const outboundTaskOrdersRelations = relations(
  outboundTaskOrders,
  ({ one }) => ({
...
export const outboundTaskItemsRelations = relations(
  outboundTaskItems,
  ({ one }) => ({
    task: one(outboundTasks, {
...
export const outboundTaskLinesRelations = relations(
  outboundTaskLines,
  ({ one }) => ({
    task: one(outboundTasks, {
...
export const outboundBatchesRelations = relations(
  outboundBatches,
  ({ one, many }) => ({
    warehouse: one(warehouses, {
...
export const fulfillmentOrderBatchesRelations = relations(
  fulfillmentOrderBatches,
  ({ one }) => ({
    fulfillmentOrder: one(fulfillmentOrders, {
...
export const purchaseOrderCartRelations = relations(
  purchaseOrderCart,
  ({ one }) => ({
    sku: one(skus, {
...
export const inboundReceiptsRelations = relations(
  inboundReceipts,
  ({ one, many }) => ({
    warehouse: one(warehouses, {
...
// export const inboundPlansRelations = relations(
//   inboundPlans,
//   ({ one, many }) => ({
//     warehouse: one(warehouses, {
...
//       references: [purchaseOrders.id],
//     }),
//     // parentPlan과 childPlans 관계 임시 제거 (self-referential 관계 문제)
...
export const inboundPlansRelations = relations(
  ...
  ({ one, many }) => ({
    ...
    // self-referential 관계는 완전히 제거
    items: many(inboundPlanItems),
  }),
);

export const inboundPlanItemsRelations = relations(
  inboundPlanItems,
  ({ one, many }) => ({
...
export const inboundReceiptLinesRelations = relations(
  inboundReceiptLines,
  ({ one }) => ({
...
// Movement Relations
export const movementJobsRelations = relations(
  movementJobs,
  ({ one, many }) => ({
    warehouse: one(warehouses, {
...
export const movementJobLinesRelations = relations(
  movementJobLines,
  ({ one }) => ({
...
export const productSkuMappingsRelations = relations(
  productSkuMappings,
  ({ one, many }) => ({
    warehouse: one(warehouses, {
...
export const productSkuMappingItemsRelations = relations(
  productSkuMappingItems,
  ({ one }) => ({
...
export const productSkuMappingSnapshotsRelations = relations(
  productSkuMappingSnapshots,
  ({ one, many }) => ({
    warehouse: one(warehouses, {
...
export const stocktakingSessionsRelations = relations(
  stocktakingSessions,
  ({ one, many }) => ({
    warehouse: one(warehouses, {
...
export const stocktakingLinesRelations = relations(
  stocktakingLines,
  ({ one }) => ({
...
export const stocktakingAdjustmentsRelations = relations(
  stocktakingAdjustments,
  ({ one }) => ({
    session: one(stocktakingSessions, {
...
export type DbTx = Parameters<
  Parameters<TypedDatabase<typeof wmsSchema>['transaction']>[0]
>[0];
