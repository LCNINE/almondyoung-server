...
async function checkProductsHaveWMTag(productIds: string[], container: any): Promise<boolean> {
  if (!productIds.length) return false;
  const productModule = container.resolve(Modules.PRODUCT);
  const products = await productModule.listProducts({ id: productIds }, { relations: ['tags'] });
  return products.some((p: any) => (p.tags ?? []).some((tag: any) => tag.value === WELCOME_MEMBERSHIP_TAG));
}

async function getOrderData(orderId: string, container: any): Promise<OrderData | null> {
...
export default async function handleWelcomeMembershipOrder({ event, container }: SubscriberArgs<{ id: string }>) {
  ...
  try {
    ...
    if (!customerId) {
      logger.info(`[WelcomeMembership] Order ${orderId} has no customer_id, skipping`);
      return;
    }
    ...
    if (!userId) {
      logger.warn(`[WelcomeMembership] Customer ${customerId} has no almond_user_id, skipping`);
      return;
    }
    ...
    if (eventName === 'order.placed') {
      ...
      logger.info(`[WelcomeMembership] Marked purchased: userId=${userId}, orderId=${orderId}`);
    } else if (eventName === 'order.canceled') {
      const hasOther = await hasOtherActiveMedusaWMOrders(customerId, orderId, container);
      if (hasOther) {
        logger.info(`[WelcomeMembership] User ${userId} has other active WM orders, not reverting`);
        return;
      }
    ...
    logger.error(`[WelcomeMembership] ${eventName} handler error for order ${orderId}:`, err);
  }
}
