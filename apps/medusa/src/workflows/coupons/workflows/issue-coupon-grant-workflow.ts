 ...
 * grant 조회로 옮겼다 — **프로덕션 요청 경로**에는 지금 링크를 읽는 코드가 한 줄도 없다
 * (의도된 예외 하나: `scripts/backfill-coupon-grants.ts` 는 1회성 이관 스크립트라 아직
 * 링크를 읽는다 — 옮길 대상이 남았는지 판정하는 것 자체가 그 스크립트의 일이다. 후속
 * contract PR 이 링크 테이블을 지우기 전에 그 스크립트가 여전히 필요한지 먼저 확인할 것).
 * 아무도 안 읽는 링크가 보이니 안 보이니를 걱정할 이유가 없어져서 이 스텝을 걷어낸다. 링크
 * 테이블 자체와 `src/links/customer-promotion.ts` 의 `defineLink` 정의는 남아 있다(스키마
