 ...
 *
 * 남은 하나는 **쿠폰 클레임**(`store/customers/me/promotions/[id]/claim/route.ts`)이다 —
 * 애초에 `visibility === 'claimable'` 인 쿠폰만 처리하므로(`claim/route.ts:37-39` — 아니면
 * 그 앞에서 거절) 이 함수가 푸는 문제(«공개» 쿠폰에서 장 유무로 오판하는 것)가 그 라우트엔
 * 아예 생기지 않는다. 대신 「이 고객이 이미 발급받았는가」를 묻는 `grantsFor`/`hasUsableGrant`
 * 를 직접 쓴다 — `grantsGovernUsage` 와는 다른 질문이다.
 *
