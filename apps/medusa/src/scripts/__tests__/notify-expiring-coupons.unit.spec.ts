...
describe('다운로드 쿠폰 만료 예정 안내', () => {
  ...
  it('회원별로 쿠폰을 묶고, 회원 id 가 없는 고객은 뺀다', () => {
    const notices = groupNotices(
      [
        { customer_id: 'cus_1', promotion_id: 'promo_a', expires_at: expires },
        { customer_id: 'cus_1', promotion_id: 'promo_b', expires_at: expires },
        { customer_id: 'cus_2', promotion_id: 'promo_a', expires_at: expires },
      ],
  ...
  it('알림 서비스 설정이 없으면 쿠폰을 선점하지 않고 건너뛴다', async () => {
    ...
    const container = {
      resolve: jest.fn((key: string) =>
        key === 'logger' ? { warn: jest.fn(), info: jest.fn() } : { claimExpiringClaimedGrants: claim },
      ),
    };
