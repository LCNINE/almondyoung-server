...
export class SearchController {
  ...
  async getTrendingKeywords(@Query() query: TrendingKeywordsQueryDto): Promise<TrendingKeywordsResponseDto> {
    return this.searchService.getTrendingKeywords(query);
  }

  @Get('suggestions')
  async suggestKeywords(@Query() query: SuggestKeywordsQueryDto): Promise<SearchSuggestionsResponseDto> {
