...
async function loadImage(fileId: string): Promise<LoadedImage> {
  ...
  const mediaType = isSupportedMediaType(headerType) ? headerType : sniffMediaType(buffer);

  if (!mediaType) {
    throw new Error(`지원하지 않는 이미지 형식입니다. (fileId: ${fileId}, type: ${headerType || '알 수 없음'})`);
  }

  if (buffer.byteLength > MAX_IMAGE_BYTES) {
    const mb = (buffer.byteLength / 1024 / 1024).toFixed(1);
    throw new Error(`이미지 한 장이 너무 큽니다. (${mb}MB — 장당 5MB 이하) 용량을 줄여 다시 올려주세요.`);
  }
...
export async function loadImageChunk(fileIds: string[]): Promise<LoadedImage[]> {
