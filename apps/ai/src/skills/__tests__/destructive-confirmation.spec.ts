...
/** 이 스펙은 어드민 도구의 확인 게이트를 본다. */
const ADMIN = [AI_SCOPE.ASSISTANT];
...
describe('파괴적 도구의 확인 게이트', () => {
  it.each(DESTRUCTIVE)('%s 는 confirmed 없이 실행되지 않는다', async (name) => {
    const result = (await runTool(name, { masterId: 'm1', versionId: 'v1', sessionId: 's1' }, ctx(), ADMIN)) as {
      ok: boolean;
  ...
  it('confirmed: false 도 거부한다', async () => {
    const result = (await runTool('delete_product', { masterId: 'm1', confirmed: false }, ctx(), ADMIN)) as {
      ok: boolean;
      needsConfirmation?: boolean;
    };
  ...
  it('confirmed 문자열 "true" 는 통과시키지 않는다', async () => {
    const result = (await runTool('delete_product', { masterId: 'm1', confirmed: 'true' }, ctx(), ADMIN)) as {
      ok: boolean;
      needsConfirmation?: boolean;
    };
  ...
  it('조회 도구는 확인 없이 그대로 돈다', async () => {
    ...
    await runTool('search_products', { keyword: 'x' }, readCtx, ADMIN);
    expect(called).toBe(true);
  });

  it('파괴적 도구의 스키마에 confirmed 가 노출된다', () => {
    const defs = buildToolDefinitions(ADMIN);
