...
function toolNamed(name: string): SkillTool {
  const tool = productCatalogSkill.tools.find((candidate) => candidate.definition.name === name);
...
function fakeCtx(options: { abortDuringGet?: boolean } = {}) {
  ...
  global.fetch = ((url: string | URL, init?: RequestInit): Promise<Response> => {
    ...
    if (init?.signal?.aborted) {
      ...
      // 진짜 fetch 도 동기 throw 가 아니라 reject 한다. 도구가 그 둘을 다르게 다루면
      // 스펙만 통과하고 실제로는 안 잡히는 일이 생긴다.
...
describe('취소 신호 전달', () => {
  it('조회 도중 취소되면 가격 저장 PUT 을 보내지 않는다', async () => {
    ...
    await expect(setPrice.execute({ versionId: 'v1', salePrice: 10000 }, ctx)).rejects.toMatchObject({
      name: 'AbortError',
    });
