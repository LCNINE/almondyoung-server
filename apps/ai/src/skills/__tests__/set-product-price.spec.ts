...
const setPrice = productCatalogSkill.tools.find((tool) => tool.definition.name === 'set_product_price')!;

function fakeCtx(basePriceRules: unknown[]) {
  const puts: unknown[] = [];

  global.fetch = ((_url: string | URL, init?: RequestInit): Promise<Response> => {
    if (init?.method === 'PUT') {
...
describe('set_product_price 검증', () => {
  it('0원은 Core 에 보내지 않고 거부한다', async () => {
    ...
    const result = await setPrice.execute({ versionId: 'v1', memberPrice: 0 }, ctx);
  ...
  it('판매가가 없는 Draft 에 멤버십가만 주면 거부한다', async () => {
    ...
    const result = await setPrice.execute({ versionId: 'v1', memberPrice: 9000 }, ctx);
