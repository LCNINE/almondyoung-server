...
export class ChannelProductsService {
  ...
  async getChannelProductsByMaster(masterId: string, tx?: DbTransaction): Promise<(ChannelProduct & {
  ...
  async getChannelProductsByChannel(channelId: string, filters?: {
    isActive?: boolean;
    search?: string;
    page?: number;
    limit?: number;
    ...
    const countQuery = client
      .select({ count: count() })
      .from(channelProducts)
      .innerJoin(productMasters, eq(channelProducts.masterId, productMasters.id));
  ...
  async updateChannelProduct(channelProductId: string, data: UpdateChannelProduct, tx?: DbTransaction): Promise<ChannelProduct> {
    if (!channelProductId) {
      throw new Error('Channel Product ID is required');
    }
  ...
  async getMergedChannelProduct(masterId: string, channelId: string, tx?: DbTransaction): Promise<{
    id: string;
    masterId: string;
    channelId: string;
    ...
    return {
      ...
      images: Array.isArray(data.master.images) ? data.master.images as string[] : [],
  ...
  async validateChannelProductSettings(masterId: string, channelId: string, settings: any, tx?: DbTransaction): Promise<{
    isValid: boolean;
    errors: string[];
  }> {
    if (!masterId || !channelId) {
  ...
  async checkChannelConstraints(channelId: string, productData: any, tx?: DbTransaction): Promise<{
    isAllowed: boolean;
    violations: string[];
  }> {
    if (!channelId) {
    ...
    if (productData) {
      ...
      if (channelInfo.supportedFeatures) {
        ...
        if (productData.images && Array.isArray(productData.images) && productData.images.length > 1) {
          if (features.multipleImages === false) {
            violations.push(`Channel ${channelInfo.type} does not support multiple images`);
          }
        }
        
        if (productData.hasOptions && features.optionProducts === false) {
          violations.push(`Channel ${channelInfo.type} does not support option products`);
        }
      }
    }
  ...
  async bulkCreateChannelProducts(masterId: string, channelConfigs: {
    channelId: string;
    name?: string;
    isActive?: boolean;
    channelSpecificData?: any;
  }[], tx?: DbTransaction): Promise<ChannelProduct[]> {
    if (!masterId) {
      throw new Error('Master ID is required');
    }
