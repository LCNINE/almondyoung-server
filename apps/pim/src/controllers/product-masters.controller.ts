...
export class ProductMastersController {
  ...
  @Post()
  @ApiOperation({ summary: '제품 마스터 생성', description: '새로운 제품 마스터를 생성합니다.' })
  @ApiBody({ type: CreateMasterDto, description: '제품 마스터 생성 정보' })
  @ApiResponse({ status: 201, description: '제품 마스터 생성 성공', type: ProductMaster })
  @ApiResponse({ status: 400, description: '잘못된 요청 데이터 (name, basePrice, pricingStrategy 필수)' })
  ...
  @Get()
  @ApiOperation({ summary: '제품 마스터 목록 조회', description: '제품 마스터 목록을 필터링 및 페이지네이션과 함께 조회합니다.' })
  ...
  @ApiQuery({ name: 'status', required: false, type: String, description: '제품 상태 필터' })
  @ApiQuery({ name: 'categoryId', required: false, type: String, description: '카테고리 ID 필터' })
  @ApiQuery({ name: 'brand', required: false, type: String, description: '브랜드 필터' })
  @ApiQuery({ name: 'pricingStrategy', required: false, type: String, description: '가격 전략 필터' })
  @ApiQuery({ name: 'search', required: false, type: String, description: '검색 키워드' })
  @ApiResponse({ status: 200, description: '제품 마스터 목록 조회 성공' })
  @ApiResponse({ status: 500, description: '서버 오류' })
  ...
  @Get(':id')
  @ApiOperation({ summary: '제품 마스터 상세 조회', description: '특정 제품 마스터의 상세 정보를 조회합니다.' })
  @ApiParam({ name: 'id', description: '제품 마스터 ID' })
  @ApiResponse({ status: 200, description: '제품 마스터 상세 조회 성공', type: MasterDetailDto })
  ...
  @Put(':id')
  @ApiOperation({ summary: '제품 마스터 수정', description: '기존 제품 마스터 정보를 수정합니다.' })
  @ApiParam({ name: 'id', description: '제품 마스터 ID' })
  @ApiBody({ type: UpdateProductMaster, description: '수정할 제품 마스터 정보' })
  @ApiResponse({ status: 200, description: '제품 마스터 수정 성공' })
  ...
  @Delete(':id')
  @ApiOperation({ summary: '제품 마스터 삭제', description: '제품 마스터를 삭제합니다.' })
  ...
  @ApiOperation({ summary: '가격 미리보기', description: '제품 마스터의 가격 전략 적용 미리보기를 제공합니다.' })
  @ApiParam({ name: 'id', description: '제품 마스터 ID' })
  @ApiResponse({ status: 200, description: '가격 미리보기 성공', type: PricePreviewDto })
  ...
  @Put(':id/pricing')
  @ApiOperation({ summary: '가격 전략 변경', description: '제품 마스터의 가격 전략을 변경합니다.' })
  @ApiParam({ name: 'id', description: '제품 마스터 ID' })
  @ApiBody({
    ...
    schema: {
      type: 'object',
      properties: {
        pricingStrategy: { type: 'string', description: '새로운 가격 전략' },
        migrationData: { description: '마이그레이션 데이터 (선택사항)' }
      },
      required: ['pricingStrategy']
    }
  })
  @ApiResponse({ status: 200, description: '가격 전략 변경 성공' })
  @ApiResponse({ status: 400, description: '잘못된 요청 데이터 (pricingStrategy 필수)' })
  @ApiResponse({ status: 404, description: '제품 마스터를 찾을 수 없음' })
  @ApiResponse({ status: 500, description: '서버 오류' })
