...
export class BannersController {
  ...
  @ApiResponse({ status: 201, description: '배너 생성 성공', type: BannerResponseDto })
  ...
  @ApiResponse({ status: 200, description: '배너 목록 조회 성공', type: [BannerResponseDto] })
  async listBannersByGroupId(
    @Param('bannerGroupId') bannerGroupId: string,
    @Query('includeInactive') includeInactive?: boolean,
  ): Promise<BannerResponseDto[]> {
    return this.bannersService.listBannersByGroupId(bannerGroupId, includeInactive === true);
  }
  ...
  @ApiResponse({ status: 200, description: '배너 조회 성공', type: BannerResponseDto })
  ...
  @ApiResponse({ status: 200, description: '배너 수정 성공', type: BannerResponseDto })
  @ApiResponse({ status: 404, description: '배너를 찾을 수 없음' })
