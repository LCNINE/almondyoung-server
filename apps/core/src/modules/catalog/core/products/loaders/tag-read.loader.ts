...
import { DbTransaction, TagReadModel } from '../../../catalog.types';
import { productTagValues, tagValues, tagGroups } from '../../../schema/catalog.schema';

@Injectable()
export class TagReadLoader {
  async getTags(tx: DbTransaction, masterId: string, versionId: string): Promise<TagReadModel[]> {
    const tagResults = await tx
      ...
      .orderBy(asc(tagGroups.displayOrder), asc(tagValues.displayOrder));
