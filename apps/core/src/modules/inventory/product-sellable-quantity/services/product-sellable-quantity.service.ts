...
export class ProductSellableQuantityService {
  ...
  async getByVariantIds(variantIds: string[], tx?: AnyTx): Promise<ProductSellableQuantityResult[]> {
   ...
   */
  async getSoldOutStateByVersionIds(versionIds: string[], tx?: AnyTx): Promise<Map<string, SoldOutState>> {
  ...
  async recalculateAndPublishForVariant(
    ...
    return this.dbService.run(
      async (trx) => {
        ...
        // 입고로 실재고가 붙는 순간 출시예정 플래그를 걷는다 (ADR-0028). 남겨두면 두 가지가 샌다 —
        // 선판매·항상판매가 계속 눌린 채로 있고, 나중에 진짜 품절됐을 때 "곧 출시 예정"이 되살아난다.
        // void 매칭은 실재고가 없어 여기 걸리지 않으므로 관리자가 직접 체크를 풀어야 한다.
        if (projection.availabilityOverride === 'coming_soon' && projection.stockBoundQuantity > 0) {
          await trx
            .update(wmsTables.salesVariantPolicies)
            .set({ availabilityOverride: null, comingSoonDate: null, updatedAt: new Date() })
        ...
        await trx
          ...
          .values({
            ...
            comingSoonDate: projection.comingSoonDate,
...
export function hasProductSellableQuantityProjectionChanged(
  ...
  return (
    ...
    (current.comingSoonDate ?? null) !== (previous.comingSoonDate ?? null)
  );
}
