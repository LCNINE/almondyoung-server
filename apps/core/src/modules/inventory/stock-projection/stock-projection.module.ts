...
import { ExpectedArrivalsReader } from './services/expected-arrivals.reader';

@Module({
  // 파이프라인 ③(이동 중 잔량)의 뺄셈은 WarehouseTransferReader 가 소유한다 — 다시 쓰지 않고 빌려온다.
  imports: [SharedModule, CoreInventoryModule, WarehouseTransferModule, ProcurementModule],
  controllers: [StockProjectionController],
  providers: [
    StockProjectionService,
    StockProjectionReader,
    StockProjectionManager,
    InboundPipelineReader,
    ExpectedArrivalsReader,
  ],
