  ...
  <Table>
    ...
    {invoices.map((invoice) => (
      <Row key={invoice.id}>
        ...
        <Cell>
          <Button onClick={() => viewDetail(invoice.id)}>상세보기</Button>
        </Cell>
      </Row>
    ))}
  </Table>
...
<PreferencesForm>
  ...
  <Description>세금계산서 신청 시 자동으로 입력될 정보입니다.</Description>
...
<CancelModal>
  ...
  <Warning>
    세금계산서를 취소하면 되돌릴 수 없습니다. 홈택스에서도 별도로 취소 처리해야
    합니다.
  </Warning>
...
const ExportButton = () => {
  ...
  const handleExport = async () => {
    try {
      ...
      toast.success(`${count}건의 세금계산서가 엑셀로 다운로드되었습니다.`);
    } catch (error) {
      console.error(error);
  ...
  return (
    <Button onClick={handleExport} disabled={loading} icon={<DownloadIcon />}>
...
const NOTIFICATION_MESSAGES = {
  CREATED:
    '세금계산서가 신청되었습니다. 발행까지 영업일 기준 2-3일 소요됩니다.',
...
<EmptyState
  ...
  action={
    <Button onClick={() => router.push('/orders')}>주문 내역 보기</Button>
  }
/>
```
