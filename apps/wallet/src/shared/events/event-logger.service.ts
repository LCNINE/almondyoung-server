...
export class EventLoggerService {
  ...
  logEventEmitted(event: BaseEvent): void {
    this.logger.log(
      `Event emitted: ${event.constructor.name}`,
      {
        eventId: event.id,
   ...
   * 이벤트 처리 재시도 로그
   */
  logEventRetry(event: BaseEvent, attempt: number, maxAttempts: number, error: string): void {
    this.logger.warn(
      `Event processing retry ${attempt}/${maxAttempts}`,
      {
        eventId: event.id,
        eventType: event.constructor.name,
        attempt,
