...
export class EventProcessorService {
  ...
  async processWithRetry<T extends BaseEvent>(
    ...
    const { maxAttempts = 3, retryDelay = 1000, exponentialBackoff = true } = options;
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
      try {
        ...
        if (attempt < maxAttempts) {
          // 재시도 로그
          this.eventLogger.logEventRetry(event, attempt, maxAttempts, error.message);
    ...
    this.eventLogger.logEventFinalFailure(event, lastError?.message || 'Unknown error');
    return result;
  }
