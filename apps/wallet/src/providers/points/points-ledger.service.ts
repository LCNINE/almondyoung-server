...
export class PointsLedgerService {
  ...
  async capture(
    ...
    if (!latestHold) {
      ...
      return this.failedResult('CAPTURE', 'POINT_HOLD_NOT_FOUND', {
        legId: req.legId,
      });
    }

    if (latestHold.status === 'CAPTURED') {
      const capturedEventId = latestHold.capturedEventId;
      return {
        resultStatus: 'CAPTURED',
        providerTransactionId: capturedEventId ?? latestHold.id,
        raw: {
          providerType: 'POINTS',
          operation: 'CAPTURE',
  ...
  async cancel(
    ...
    const latestHold = await this.findLatestHoldByLegId(tx, req.legId);
    if (!latestHold) {
      return this.failedResult('CANCEL', 'POINT_HOLD_NOT_FOUND', {
        legId: req.legId,
      });
    }
  ...
  async refund(
    ...
    if (legTotals.redeemedAmount <= 0) {
      return this.failedResult('REFUND', 'NO_CAPTURED_REDEEM', {
        legId: req.legId,
      });
    }
    ...
    if (redeemDetails.length === 0) {
      return this.failedResult('REFUND', 'REDEEM_DETAILS_NOT_FOUND', {
        legId: req.legId,
      });
    }
    ...
    const refundedByDetail = await this.readRefundedAmountByRedeemDetailIds(
      tx,
      redeemDetails.map((detail) => detail.redeemDetailId),
    );
  ...
  async getTransaction(
    ...
    if (latestHold) {
      ...
      const totals = await this.readPointLegTotals(tx, latestHold.userId, req.legId);
      if (totals.redeemedAmount > 0 && totals.refundedAmount >= totals.redeemedAmount) {
        return {
    ...
    const pointEventRows = await tx
      .select({
        userId: pointEvents.userId,
      })
      .from(pointEvents)
      .where(eq(pointEvents.legId, req.legId))
      .limit(1);
    const userId = pointEventRows[0]?.userId;
    ...
    if (!userId) {
      return {
        ...
        raw: {
          ...
          reason: 'NO_HOLD_AND_NO_EVENTS',
        },
      };
    }

    const totals = await this.readPointLegTotals(tx, userId, req.legId);
    if (totals.redeemedAmount > 0 && totals.refundedAmount >= totals.redeemedAmount) {
      return {
        status: 'REFUNDED',
        raw: {
  ...
  private async readBalance(
    ...
    const confirmedRows = await tx
      .select({
        amount: sql<number>`coalesce(sum(${pointEvents.amount}), 0)`,
      })
      .from(pointEvents)
    ...
    const reservedRows = await tx
      .select({
        amount: sql<number>`coalesce(sum(${pointHolds.amount}), 0)`,
      })
      .from(pointHolds)
  ...
  private async readRefundedAmountByRedeemDetailIds(
    ...
    const refundedByDetail = new Map<string, number>();
    for (const row of rows) {
      if (!row.redeemDetailId) {
        continue;
      }

      refundedByDetail.set(row.redeemDetailId, Number(row.refundedAmount ?? 0));
    }

    return refundedByDetail;
  }
  ...
  private buildAuthorizeResultFromHold(
    ...
    return this.failedResult('AUTHORIZE', 'POINT_HOLD_ALREADY_CANCELLED', {
      holdId,
    });
  }
