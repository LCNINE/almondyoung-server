 ...
 *
 * 실패를 두 갈래로 나눠 내려주되, «둘 다 등록은 막는다». 확인되지 않은 계좌를 통과시키면
 * 이 조회를 도입한 이유가 사라진다 — 고객은 등록된 줄 알았다가 D+1 에 거절을 통보받는다.
 * 구분은 «무엇을 고쳐야 하는지»를 말하기 위한 것이다: MISMATCH 는 틀린 칸으로 되돌리고,
 * UNAVAILABLE 은 고칠 것이 없으니 잠시 후 다시 시도하게 한다.
 *
 * 문서의 지원 은행 목록에 카카오뱅크·토스뱅크가 없지만 실측으로는 조회된다. 즉 UNAVAILABLE
 * 은 사실상 장애·일시 오류일 때만 난다.
 */
@Injectable()
