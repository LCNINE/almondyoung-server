...
import {
  BlacklistsAlreadyExistsException,
  BlacklistsNotFoundException,
} from './exceptions/blacklists.exceptions';
...
export class BlacklistsService {
  ...
  async getBlacklists(
    ...
    if (filters?.userId) {
      whereConditions.push(
        eq(userServiceSchema.blacklists.userId, filters.userId),
      );
    }
  ...
  async getBlacklistByUserId(
    userId: string,
    tx?: DbTransaction,
  ): Promise<BlacklistsResponseDto | null> {
    const client = this.getClient(tx);
  ...
  async createBlacklist(
    blacklistsCreateDto: BlacklistsCreateDto,
    adminId: string,
    tx?: DbTransaction,
  ): Promise<void> {
    const client = this.getClient(tx);
    // 해당 블랙리스트에있는 사용자가 있는지 조회
    const existingBlacklist = await this.getBlacklistByUserId(
      blacklistsCreateDto.userId,
      tx,
    );

    if (existingBlacklist?.deletedAt) {
      throw new BlacklistsAlreadyExistsException(
        '해당 유저의 블랙리스트 정보가 이미 존재합니다.',
      );
    }
  ...
  async deleteBlacklist(
    ...
    if (!existingBlacklist) {
      throw new BlacklistsNotFoundException(
        '해당 블랙리스트 정보를 찾을 수 없습니다.',
      );
    }
    await client
