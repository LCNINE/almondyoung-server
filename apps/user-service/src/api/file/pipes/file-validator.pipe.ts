...
import { ALLOWED_MIME_TYPES, FILE_SIZE_LIMIT } from '../../../constants/file.constants';
...
export class FileValidatorPipe implements PipeTransform {
  async transform(file: MultipartFile): Promise<ValidatedFile> {
    ...
    if (buffer.length > FILE_SIZE_LIMIT) {
      throw new BadRequestException(`파일 크기는 ${FILE_SIZE_LIMIT} 를 초과할 수 없습니다.`);
    }

    // MIME 타입 검증
    if (!ALLOWED_MIME_TYPES.includes(file.mimetype as (typeof ALLOWED_MIME_TYPES)[number])) {
      throw new BadRequestException(`지원하지 않는 파일 형식입니다. 허용된 형식: ${ALLOWED_MIME_TYPES.join(', ')}`);
    }
    ...
    if (!validFolderNames.includes(folderName)) {
      throw new BadRequestException(`Invalid folderName. Allowed values: ${validFolderNames.join(', ')}`);
    }
