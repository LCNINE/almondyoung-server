...
export class ShopService {
  ...
  async createShopInfo(createShopDto: CreateShopInfoDto, userId: string): Promise<void> {
    ...
    if (existingShop) {
      ...
      await this.dbService.db.update(schema.shops).set(shopData).where(eq(schema.shops.userId, userId));
    } else {
      // 완전 신규 유저 → insert
      await this.dbService.db.insert(schema.shops).values({ userId, ...shopData });
    }
  }

  async updateShopInfo(updateShopDto: UpdateShopInfoDto, userId: string): Promise<void> {
    const shopData = {
  ...
  async findOneByUserId(userId: string): Promise<schema.Shop | null> {
    const [shop] = await this.dbService.db.select().from(schema.shops).where(eq(schema.shops.userId, userId));

    return shop ?? null;
  }

  async findOneByShopIdAndUserId(shopId: string, userId: string): Promise<schema.Shop | undefined> {
  ...
  async updateRemindAt(userId: string) {
    ...
    if (existingShop) {
      await this.dbService.db.update(schema.shops).set({ remind_at: remindAt }).where(eq(schema.shops.userId, userId));
    } else {
      await this.dbService.db.insert(schema.shops).values({ userId, remind_at: remindAt });
    }
  }
}
