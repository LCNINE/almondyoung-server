...
export class ProviderController {
  ...
  async findAll(@Query(ValidationPipe) filter: ProviderFilterDto) {
    ...
    return this.db.query.notificationProviders.findMany({
      ...
      orderBy: [asc(notificationProviders.channel), desc(notificationProviders.priority)],
    });
  }
  ...
  async create(@Body(ValidationPipe) dto: CreateProviderDto) {
    ...
    const [provider] = await this.db.insert(notificationProviders).values(newProvider).returning();
  ...
  @ApiResponse({ status: 404, description: '프로바이더를 찾을 수 없음' })
  async update(@Param('id') id: string, @Body(ValidationPipe) dto: UpdateProviderDto) {
