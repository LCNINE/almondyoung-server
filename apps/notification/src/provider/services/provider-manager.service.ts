...
export class ProviderManagerService implements OnModuleInit, OnModuleDestroy {
  ...
  private async loadProviders() {
    ...
    const dbProviders = [
      ...
      {
        providerId: 'c1f4b8de-6a3f-4d1e-9c7a-2e5b8f0a4d13',
        providerName: 'NHN SMS',
        isAvailable: true,
        channel: Channel.SMS,
        priority: 10,
        config: {
          apiUrl: this.configService.get<string>('NHN_SMS_API_URL') || 'https://sms.api.nhncloudservice.com',
          appKey: this.configService.get<string>('NHN_SMS_APP_KEY') || '',
          secretKey: this.configService.get<string>('NHN_SMS_SECRET_KEY') || '',
          sendNo: this.configService.get<string>('NHN_SMS_SEND_NO') || '',
          timeout: 30000,
        },
      },
      {
