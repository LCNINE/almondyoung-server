...
export class NotificationProcessor {
  ...
  async handleScheduledCheck(job: Job) {
    const pendingNotifications = await this.db.query.notifications.findMany({
      where: and(eq(notifications.status, NotificationStatus.PENDING), lte(notifications.sendAt, new Date())),
      limit: 100,
    });
    ...
    for (const notification of pendingNotifications) {
      ...
      if (updated && job.queue) {
        ...
        await job.queue.add('send-notification', { notificationId: notification.notificationId }, { priority });
      }
    }
  }
