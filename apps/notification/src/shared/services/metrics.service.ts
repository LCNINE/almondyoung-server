...
export class MetricsService {
    ...
    async getDailyMetrics(date: Date) {
        ...
        const result = await this.db
            ...
            .leftJoin(
                notificationLogs,
                eq(notifications.notificationId, notificationLogs.notificationId)
            )
            .where(
                between(notifications.createdAt, startOfDay, endOfDay)
            )
    ...
    async getChannelPerformance(channel: string, days = 7) {
        ...
        const performance = await this.db
            ...
            .where(
                and(
                    eq(notifications.channel, channel as any),
    ...
    async getProviderHealth() {
        ...
        `,
            ...
            .where(
                sql`${notificationLogs.createdAt} >= NOW() - INTERVAL '1 hour'`
            )
            .groupBy(
                notificationLogs.provider,
