...
export class WebhookService {
        ...
        try {
            ...
            const notification = await this.db.query.notifications.findFirst({
                where: and(
                    ...
                    sql`metadata->>'messageSid' = ${messageSid}`
                ),
            });
            ...
            if (!notification) {
                const receipt = await this.db.query.receipts.findFirst({
                    where: and(
                        eq(receipts.provider, 'twilio'),
                        sql`provider_response->>'sid' = ${messageSid}`
                    ),
                });
    ...
    async handleKakaoWebhook(
        ...
        try {
            ...
            this.logger.error('Failed to process Kakao webhook', {
                error: error.message,
            }, error.stack);
    ...
    private async processKakaoMessageResultUpdate(webhookData: any): Promise<void> {
        ...
        for (const hook of hooks) {
            ...
            if (!notification) {
                const receipt = await this.db.query.receipts.findFirst({
                    where: and(
                        eq(receipts.provider, 'nhn-kakao'),
                        sql`provider_response->>'requestId' = ${requestId}`
                    ),
                });
