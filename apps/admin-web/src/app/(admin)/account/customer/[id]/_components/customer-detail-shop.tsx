...
export function CustomerDetailShopContent({ userId }: { userId: string }) {
  ...
  const rows: { key: string; value: string | null }[] = [
    ...
    { key: '운영 기간', value: shop.yearsOperating ? `${shop.yearsOperating}년` : null },
    { key: '샵 유형', value: shop.shopType ? shopTypeLabels[shop.shopType] ?? shop.shopType : null },
    { key: '카테고리', value: Array.isArray(shop.categories) ? shop.categories.join(', ') : null },
    { key: '타겟 고객', value: Array.isArray(shop.targetCustomers) ? shop.targetCustomers.join(', ') : null },
    { key: '운영 요일', value: Array.isArray(shop.openDays) ? shop.openDays.join(', ') : null },
