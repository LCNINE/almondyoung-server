...
describe('demo console proxy', () => {
  ...
  it.each(['practice', 'replenishment'])(
    'allows authenticated same-origin %s preparation and rejects a cross-origin request',
    async (route) => {
      ...
      expect(
        (await POST(request('POST'), params('core', [route]))).status
      ).toBe(200);
      expect(transport.mock.calls[0][0]).toBe(
        `https://core.almondyoung-next.com/demo/${route}`
      );
      transport.mockClear();
      expect(
        (
