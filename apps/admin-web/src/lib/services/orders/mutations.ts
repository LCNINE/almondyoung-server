...
export const useUpdateOutboundBatch = () => {
    ...
    return useMutation({
        ...
        onSuccess: (_, { id }) => {
            queryClient.invalidateQueries({ queryKey: orderQueryKeys.outboundBatches });
            queryClient.invalidateQueries({ queryKey: orderQueryKeys.outboundBatch(id) });
        },
    });
};
...
export const useUpdateVariantMatching = () => {
    ...
    return useMutation({
        mutationFn: ({ variantId, data }: { variantId: string; data: Partial<VariantMatchingDto> }) =>
...
export const useIgnoreMatching = () => {
    ...
    return useMutation({
        mutationFn: ({ id, stockPolicy }: { id: string; stockPolicy?: StockPolicyDto }) =>
