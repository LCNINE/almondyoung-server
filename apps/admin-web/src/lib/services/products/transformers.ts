 ...
 */
export const getChannelMarkType = (
  channelName: string
): 'almondyoung' | 'coupang' | 'naver_smartstore' | 'phone_order' | 'other' => {
  const name = channelName.toLowerCase();

  if (
    name.includes('아몬드영') ||
    name.includes('almondyoung') ||
    name.includes('공식')
  ) {
    return 'almondyoung';
  }
  ...
  if (
    name.includes('네이버') ||
    name.includes('naver') ||
    name.includes('스마트스토어') ||
    name.includes('smartstore')
  ) {
