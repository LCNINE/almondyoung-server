...
export const useChannelProductsByChannel = (query: ChannelProductsQuery) => {
    return useQuery({
        ...
        queryFn: () => products.channelProducts.getByChannel(query.channelId, query),
 ...
 * 병합된 채널 제품 조회
 */
export const useMergedChannelProduct = (masterId: string, channelId: string) => {
    return useQuery({
...
export const useMatchingTable = (query: Record<string, string> = {}) => {
    return useQuery({
        ...
        queryFn: async () => {
            ...
            const response = await fetch('/api/matching-table?' + new URLSearchParams(query));
