...
// SKU 생성
export const createSku = async (data: CreateSkuDto): Promise<SkuResponseDto> => {
...
// SKU 검색
export const getSkus = async (query: SkuQuery = {}): Promise<SkuResponseDto[]> => {
...
// SKU 수정
export const updateSku = async (id: string, data: UpdateSkuDto): Promise<SkuResponseDto> => {
...
// SKU에 바코드 추가
export const addBarcode = async (id: string, data: AddBarcodeDto): Promise<void> => {
    await client.post(
...
// 바코드 제거
export const removeBarcode = async (id: string, barcodeId: string): Promise<void> => {
    await client.delete(
...
// SKU 재고 요약 조회
export const getSkuStockSummary = async (id: string): Promise<SkuStockSummaryDto> => {
    const response = await client.get(
