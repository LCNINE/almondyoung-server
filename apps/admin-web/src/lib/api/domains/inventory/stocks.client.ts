...
// 재고 조회
export const getStocks = async (query: StockQuery = {}): Promise<StockDto[]> => {
...
// SKU별 총 재고 조회
export const getSkuTotalStock = async (skuId: string): Promise<SkuTotalStockDto> => {
    const response = await client.get(
...
// 재고 이벤트 이력 조회
export const getStockHistory = async (query: StockHistoryQuery): Promise<StockHistoryDto[]> => {
    const response = await client.get(
...
// 재고 현황 재구축
export const rebuildStockSummary = async (skuId: string, warehouseId: string): Promise<void> => {
    await client.post(
