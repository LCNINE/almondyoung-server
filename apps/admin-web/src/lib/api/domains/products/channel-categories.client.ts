...
export const channelCategoriesClient = {
  listChannelCategories: async (): Promise<ChannelCategoryListResponseDto> => {
    const response = await client.get(`${ALMONDYOUNG_API_BASE_URL}/channels/categories`);
    return response.data;
  },

  getChannelCategory: async (id: string): Promise<ChannelCategoryDto> => {
  ...
  deleteChannelCategory: async (id: string): Promise<void> => {
    await client.delete(
      `${ALMONDYOUNG_API_BASE_URL}/channels/categories/${encodeURIComponent(id)}`
    );
  },
