...
const getProduct: SkillTool = {
  definition: {
    name: 'get_product',
    description:
      '상품 상세를 본다. 수정하기 전에 현재 값과 버전 상태(active/draft)를 확인하는 데 쓴다.',
    input_schema: {
      type: 'object',
...
const updateDraft: SkillTool = {
    ...
    input_schema: {
      ...
      properties: {
        ...
        descriptionHtml: {
          type: 'string',
          description:
            '옛 HTML 본문 필드. 마크다운 본문은 description 에 넣는다 — 여기 넣지 않는다.',
        },
        brand: { type: 'string' },
        ...
        isVisibleToMembersOnly: {
          type: 'boolean',
          description:
            '멤버십 회원 전용 노출. true 면 비회원의 목록·검색·상세에서 숨겨진다.',
        },
        isOverseas: {
          type: 'boolean',
          description:
            '해외직구 상품. true 면 결제할 때 개인통관고유부호를 받는다.',
        },
      },
  ...
  async execute(input, ctx) {
    ...
    const { masterId: _m, versionId: _v, ...body } = input as Record<string, unknown>;
...
function isSimpleOverride(rules: PricingRule[]): boolean {
  ...
  return (
    rules.length === 1 &&
    rules[0].scopeType === 'all_variants' &&
    rules[0].operationType === 'override'
  );
}
...
const writeDescription: SkillTool = {
  ...
  async execute(input, ctx) {
    ...
    if (!Array.isArray(fileIds) || fileIds.length === 0) {
      return toolError(
        'fileIds 가 필요하다. upload_product_image 로 이미지를 먼저 올려 fileId 를 얻는다.'
      );
    }
    ...
    input_schema: {
      ...
      properties: {
        ...
        variantCode: {
          type: 'string',
          description:
            '품목 코드(바코드). 채널 어댑터가 이 값으로 외부 채널 상품과 매칭한다.',
        },
      },
...
const uploadImage: SkillTool = {
    ...
    input_schema: {
      ...
      properties: {
        fileName: {
          type: 'string',
          description:
            '첨부가 여러 개일 때 올릴 파일명. 생략하면 첨부된 이미지를 전부 올린다.',
        },
      },
      required: [],
    },
  ...
  async execute(input, ctx) {
    ...
    if (images.length === 0) {
      return toolError(
        '이번 메시지에 이미지 첨부가 없다. 사용자에게 이미지를 첨부해 달라고 요청한다.'
      );
    }
...
const unpublishProduct: SkillTool = {
  ...
  definition: {
    ...
    description:
      '판매를 중단한다(Active 버전을 내린다). 상품을 지우지 않고 노출만 멈추고 싶을 때 삭제 대신 쓴다.',
    input_schema: {
      type: 'object',
      properties: { masterId: { type: 'string' } },
      required: ['masterId'],
