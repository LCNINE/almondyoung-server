...
const uploadForm: SkillTool = {
  ...
  async execute(input, ctx) {
    const file = ctx.attachments.find((a) =>
      a.fileName.toLowerCase().endsWith('.xlsx')
    );
    if (!file) {
      return toolError(
        '이번 메시지에 .xlsx 첨부가 없다. 사용자에게 양식 파일을 첨부해 달라고 요청한다.'
      );
    }
...
const uploadImages: SkillTool = {
  ...
  async execute(input, ctx) {
    ...
    if (ctx.attachments.length === 0) {
      return toolError(
        '이번 메시지에 첨부 파일이 없다. 사용자에게 이미지 파일을 첨부해 달라고 요청한다.'
      );
    }
    ...
    return {
      ...
      ignoredAttachments: ctx.attachments
        .map((a) => a.fileName)
        .filter((n) => !usedNames.has(normalizeFileName(n))),
