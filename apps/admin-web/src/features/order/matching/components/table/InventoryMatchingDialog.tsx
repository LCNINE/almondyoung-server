...
/** 재고 생성/상품매칭 다이얼로그 (자동/수동/비매칭) */
export function InventoryMatchingDialog({
  isOpen,
  onClose,
  line,
}: InventoryMatchingDialogProps) {
  ...
  const { data: skuResults, isLoading: searching } = useSkuSearch(
    debounced,
    1,
    20
  );
  ...
  useEffect(() => {
    ...
    const optionLabel =
      variant?.optionKey && Object.keys(variant.optionKey).length > 0
  ...
  const sellingProductOption =
    variant?.optionKey && Object.keys(variant.optionKey).length > 0
      ? Object.entries(variant.optionKey)
          .map(([k, v]) => `${k}: ${v}`)
          .join(', ')
  ...
  const handleUpdateQuantity = (skuId: string, value: number) => {
    ...
    setLinkedSkus(
      linkedSkus.map((s) => (s.skuId === skuId ? { ...s, quantity } : s))
    );
  };
  ...
  const filteredSkuResults =
    (skuResults as any)?.items ?? (Array.isArray(skuResults) ? skuResults : []);
      ...
      <DialogContent className="max-w-5xl max-h-[90vh] p-0 flex flex-col">
        ...
        {!line.matchingId && (
          <div className="mx-6 mt-3 shrink-0 rounded-md border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700">
            이 주문의 매칭 레코드가 없습니다. PIM에서 상품 이벤트가 누락되었을
            수 있습니다. 관리자에게 문의하세요.
          </div>
        )}
            ...
            <div className="space-y-6 py-6">
              ...
              <div className="rounded-md border p-3 bg-orange-50 text-[13px] leading-5">
                <div className="font-medium mb-1">
                  1. 자동 SKU 구성 매칭 : 재고가 생성된 적이 없는 모든 재고 관리
                  상품
                </div>
                <div>
                  자동으로 재고파악 시 예상 상품으로 재고파악이 자동으로
                  매칭됩니다.
                </div>

                <div className="font-medium mt-2 mb-1">
                  2. 수동 SKU 구성 매칭 : 세트, 묶음한 상품 등 재고가 1:1 이
                  아닌 상품, 또는 기존에 재고가 있는 상품
                </div>
                <div>
                  수동 SKU 구성 매칭 선택 후 [상품명-재고명] 1:1 확인 후
                  지정하세요.
                </div>

                <div className="font-medium mt-2 mb-1">
                  3. 재고상품 비매칭 : SKU 구성 매칭을 만들지 않는 상품매칭 전략
                </div>
                <div>
                  이 판매상품 variant를 재고상품과 연결하지 않을 때
                  선택해주세요.
                </div>
              </div>
                ...
                <FormField label="재고소유" required>
                  <div className="flex gap-2">
                    ...
                    <Button
                      variant="secondary"
                      size="sm"
                      onClick={() => setShowHolderSearch(true)}
                    >
                      검색
                    </Button>
                    <Button
            ...
            <div className="space-y-6 py-6">
              ...
              <div>
                ...
                <div className="flex gap-2 mb-3">
                  <FormSelect
                    options={[{ value: 'name', label: '재고 상품명' }]}
                ...
                <div className="bg-white border rounded-lg">
                  <table className="w-full">
                    <thead className="bg-gray-50 border-b">
                      <tr>
                        <th className="text-left px-4 py-2 text-xs font-medium text-gray-700">
                          등록
                        </th>
                        <th className="text-left px-4 py-2 text-xs font-medium text-gray-700">
                          재고상품 명
                        </th>
                        <th className="text-center px-4 py-2 text-xs font-medium text-gray-700">
                          공급처
                        </th>
                      </tr>
                    </thead>
                    <tbody>
                      {searching ? (
                        ...
                        filteredSkuResults.map((sku: any) => {
                          const isLinked = linkedSkus.some(
                            (s) => s.skuId === sku.id
                          );
                          return (
                            <tr
                            ...
                            >
                              <td className="px-4 py-3">
                                <Button
                                  ...
                                  onClick={() =>
                                    !isLinked && handleAddSku(sku.id, sku.name)
                                  }
                                  disabled={isLinked}
                                >
                                  {isLinked ? '등록됨' : '등록'}
              ...
              <div className="flex flex-col items-center justify-center py-20">
                <div className="text-center space-y-3 mb-8">
                  ...
                  <div className="text-sm text-gray-500">
                    SKU 구성 매칭 없이 전략 결정 완료로 처리할 때 선택해주세요.
                  </div>
                </div>
              </div>
            </div>
