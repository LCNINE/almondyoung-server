...
const PLANNED_CHANNELS: { channel: NotificationChannel; reason: string }[] = [
  { channel: 'KAKAO', reason: '카카오 검수를 받은 알림톡 템플릿을 연결해야 합니다' },
  { channel: 'SMS', reason: '이 알림은 아직 받는 사람의 휴대폰 번호를 함께 넘기지 않습니다' },
];
...
export default function NotificationDetailTemplate({ eventKey }: { eventKey: string }) {
  ...
  const known = [...new Set([...Object.keys(template.variablesSchema ?? {}), ...variableNames(`${savedSubject} ${savedBody}`)])];
  ...
  const save = () =>
    updateTemplate.mutate(
      { templateId: template.templateId, contents: withChannelBody(template.contents, 'EMAIL', draft) },
      {
        ...
        <section className="flex flex-col gap-3 rounded-md border px-4 py-3">
          <div className="flex items-center gap-4">
            ...
            <label className="flex items-center gap-2 text-sm">
              <Checkbox checked={emailOn} disabled={updateEvent.isPending} onCheckedChange={(v) => setEmail(v === true)} />
              {CHANNEL_LABEL.EMAIL}
            </label>
        ...
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
          <section className="flex flex-col gap-3">
            ...
            <div className="flex flex-wrap items-center gap-1.5">
              ...
              {known.map((name) => (
                <Button key={name} type="button" variant="outline" size="sm" className="h-7 px-2 text-xs" onClick={() => insertVariable(name)}>
                  {`{{${name}}}`}
                </Button>
              ))}
            </div>
            {unknown.length > 0 && (
              <p className="text-destructive text-xs">
                이 알림이 채워주지 않는 변수가 있습니다: {unknown.map((n) => `{{${n}}}`).join(', ')} — 발송하면 빈칸으로 나갑니다.
              </p>
            )}
          ...
          <section className="flex flex-col gap-3">
            ...
            <MessagePreview channel="EMAIL" subject={draft.subject} body={draft.body} />
