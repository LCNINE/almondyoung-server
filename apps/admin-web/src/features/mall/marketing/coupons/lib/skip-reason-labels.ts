...
const LABELS: Record<string, string> = {
  ...
  // 🔴 `link_error` 는 **쿠폰이 발급된 뒤** 표시용 링크 생성만 실패한 경우다 — 그 시점엔
  // grant 가 이미 있다. 옛 문구(「발급 처리 중 오류」)는 관리자에게 «발급 실패» 로 읽혀
  // 실제 상태와 반대였다.
  //
  // ⚠️ ADR-0034(발급이 워크플로를 지난다) 이후 **서버는 이 사유를 더 이상 내보내지 않는다** —
  // 링크가 실패하면 장까지 함께 되감기므로 「발급됐는데 안 보임」 상태 자체가 없고, 실패는
  // `grant_error` 로 나간다. 그래도 항목을 지우지 않는 이유는 배포 롤오버다: 새 admin-web 이
  // 옛 core 와 잠시 공존하는 동안에는 이 사유가 실제로 도착할 수 있다.
