    ...
    <Dialog open={open} onOpenChange={(v) => (v ? onOpenChange(true) : handleClose())}>
      <DialogContent className="max-h-[90vh] w-[calc(100vw-2rem)] gap-0 overflow-hidden p-0 sm:max-w-[640px]">
        <DialogHeader className="border-b px-6 py-5">
          ...
          <DialogDescription>
            메타데이터를 등록합니다. 파일은 등록 후 file-service 의 fileId 로 버전을 추가하거나, 함께 입력할 수 있습니다.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="grid max-h-[calc(90vh-132px)] gap-5 overflow-y-auto px-6 py-5">
          ...
          <div className="grid gap-3 rounded-md border bg-muted/20 p-4">
            ...
            <p className="text-xs text-muted-foreground">
              file-service 에 이미 업로드된 파일 ID 가 있다면 입력하세요. 비워두면 등록 후 파일 버전 추가 화면에서 등록합니다.
            </p>
            <div className="grid gap-2">
              ...
              <Input
                ...
                onChange={(e) => setForm((p) => ({ ...p, initialFileId: e.target.value }))}
              />
            </div>
            <div className="grid gap-2">
              ...
              <Input
                ...
                onChange={(e) =>
                  setForm((p) => ({ ...p, initialReleaseNote: e.target.value }))
                }
              />
            </div>
          </div>
