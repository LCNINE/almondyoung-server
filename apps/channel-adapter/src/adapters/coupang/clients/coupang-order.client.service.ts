...
export class CoupangOrderClient extends CoupangBaseClient {
   ...
   * @returns API 응답 데이터
   */
  async getOrderSheets(params: GetOrderSheetsParams): Promise<CoupangOrderSheetListResponse> {
    try {
      ...
      const authorization = this.generateAuthHeader(config.accessKey, config.secretKey, 'GET', path, queryString);
   ...
   * @returns API 응답 데이터
   */
  async getSingleOrderSheet(shipmentBoxId: string | number): Promise<CoupangSingleOrderSheetResponse> {
    try {
      const config = this.getApiConfig();

      this.logger.log(`🔍 쿠팡 발주서 단건 조회 (shipmentBoxId): ${shipmentBoxId}`);
      ...
      this.logger.error(`❌ 쿠팡 발주서 단건 조회 실패 (${shipmentBoxId}):`, error);
   ...
   * @returns API 응답 데이터
   */
  async getSingleOrderSheetByOrderId(orderId: string | number): Promise<CoupangOrderSheetByOrderIdResponse> {
    try {
      const config = this.getApiConfig();
      ...
      this.logger.log(`✅ 쿠팡 발주서 단건 조회 (orderId) 성공: ${orderId} (${response.data || 0}건)`);
      return response.data;
    } catch (error) {
      this.logger.error(`❌ 쿠팡 발주서 단건 조회 (orderId) 실패 (${orderId}):`, error);
  ...
  async getAllOrderSheetsByStatus(
    ...
    do {
      try {
        ...
        this.logger.log(`📄 페이지 조회 완료: ${response.data.length}건 (누적: ${allOrderSheets.length}건)`);
        ...
        this.logger.error(`❌ 발주서 페이징 조회 실패 (nextToken: ${nextToken}):`, error);
        throw error;
      }
    } while (nextToken);
  ...
  async acknowledgeOrdersheets(
    ...
    // 요청 파라미터 검증
    const parsedReq = CoupangAcknowledgeOrdersheetsRequestSchema.safeParse(payload);
    if (!parsedReq.success) {
      const flattenedErrors = parsedReq.error.flatten();
      this.logger.error('상품준비중 처리 요청 파라미터 검증 실패:', flattenedErrors);
    ...
    this.logger.log(`📦 쿠팡 상품준비중 처리 요청 (vendorId=${vendorId}, count=${shipmentBoxIds.length})`);
    ...
    try {
      ...
      this.logger.log(`✅ 상품준비중 처리 성공 (전체 상태 코드=${parsedRes.data.responseCode})`);
   ...
   */
  async uploadInvoices(payload: CoupangUploadInvoiceRequest): Promise<CoupangUploadInvoiceResponse> {
    // 요청 파라미터 검증
    const parsedReq = CoupangUploadInvoiceRequestSchema.safeParse(payload);
    if (!parsedReq.success) {
      const flattenedErrors = parsedReq.error.flatten();
      this.logger.error('송장 업로드 처리 요청 파라미터 검증 실패:', flattenedErrors);
      throw new BadRequestException({
    ...
    this.logger.log(`📦 쿠팡 송장 업로드 처리 요청 (vendorId=${vendorId}, count=${orderSheetInvoiceApplyDtos.length})`);
    ...
    try {
      ...
      this.logger.log(`✅ 송장 업로드 처리 성공 (전체 상태 코드=${parsedRes.data.responseCode})`);
      return parsedRes.data;
    } catch (error) {
      if (error.response) {
        this.logger.error(
   ...
   */
  async updateInvoices(payload: CoupangUpdateInvoiceRequest): Promise<CoupangUpdateInvoiceResponse> {
    // 요청 파라미터 검증
    const parsedReq = CoupangUpdateInvoiceRequestSchema.safeParse(payload);
    if (!parsedReq.success) {
      const flattenedErrors = parsedReq.error.flatten();
      this.logger.error('송장 업데이트 처리 요청 파라미터 검증 실패:', flattenedErrors);
      throw new BadRequestException({
    ...
    try {
      ...
      this.logger.log(`✅ 송장 업데이트 처리 완료 (전체 상태 코드=${response.data.data.responseCode})`);
      return response.data;
    } catch (error) {
      if (error.response) {
        this.logger.error(
  ...
  async getDeliveryHistory(shipmentBoxId: string | number): Promise<CoupangDeliveryHistoryResponse> {
    try {
      ...
      if (error.response) {
        ...
        this.logger.error(`❌ 쿠팡 배송상태 히스토리 조회 실패: ${error.message}`);
      }
