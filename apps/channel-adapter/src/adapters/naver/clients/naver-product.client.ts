...
export class NaverProductClient extends NaverBaseClient {
   ...
   * @returns API 응답 데이터
   */
  async changeSaleStatus(originProductNo: number, body: ChangeSaleStatusBody): Promise<any> {
    ...
    if (!parsedBody.success) {
      ...
      this.logger.error('❌ 판매 상태 변경 요청 파라미터 검증 실패:', flattenedErrors);
   ...
   * @returns API 응답 데이터
   */
  async updateOptionStock(originProductNo: number, body: UpdateOptionStockBody): Promise<any> {
    const token = await this.authService.getAccessToken(); // 🔑 인증 서비스 사용
    ...
    if (!parsedBody.success) {
      const flattenedErrors = parsedBody.error.flatten();
      this.logger.error('❌ 옵션 재고 업데이트 요청 파라미터 검증 실패:', flattenedErrors);
      throw new BadRequestException({
