...
async function testCoupangSingleOrderSheet() {
  // 환경변수 확인
  const requiredEnvs = ['COUPANG_VENDOR_ID', 'COUPANG_ACCESS_KEY', 'COUPANG_SECRET_KEY'];
  ...
  console.log(`  - Access Key: ${process.env.COUPANG_ACCESS_KEY?.slice(0, 8)}...`);
  ...
  try {
    ...
    console.log(`  👤 수취인: ${orderEvent.buyer?.name} (${orderEvent.buyer?.contact})`);
    console.log(`  🏠 배송지: ${orderEvent.buyer?.address?.roadAddress} ${orderEvent.buyer?.address?.detailAddress}`);
    console.log(`  📮 우편번호: ${orderEvent.buyer?.address?.postalCode}`);

    if (orderEvent.dispatch?.trackingNumber) {
      console.log(`  🚚 운송장: ${orderEvent.dispatch.deliveryCompanyCode} - ${orderEvent.dispatch.trackingNumber}`);
    ...
    if (orderEvent.discountAmount && orderEvent.discountAmount > 0) {
      console.log(`  💸 할인금액: ${orderEvent.discountAmount.toLocaleString()}원`);
    }
    ...
    if (error.message.includes('not found') || error.message.includes('찾을 수 없')) {
      console.log('\n💡 문제 해결 방법:');
      console.log('  1. shipmentBoxId 확인 (Wing 또는 발주서 목록 API에서 조회 가능)');
