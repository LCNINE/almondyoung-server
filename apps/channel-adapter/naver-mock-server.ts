 ...
 */
app.post(
  '/v1/pay-order/seller/product-orders/:productOrderId/cancel/approve',
  validateToken,
  (req, res) => {
    console.log(
      `❌ 네이버 취소승인 Mock API 호출 - 상품주문ID: ${req.params.productOrderId}`,
    );
 ...
 */
app.post(
  '/v1/pay-order/seller/product-orders/:productOrderId/return/approve',
  validateToken,
  (req, res) => {
    console.log(
      `🔄 네이버 반품승인 Mock API 호출 - 상품주문ID: ${req.params.productOrderId}`,
    );
 ...
 */
app.post(
  '/v1/pay-order/seller/product-orders/:productOrderId/exchange/approve',
  validateToken,
  (req, res) => {
    console.log(
      `🔄 네이버 교환승인 Mock API 호출 - 상품주문ID: ${req.params.productOrderId}`,
    );
...
app.get('/', (req, res) => {
  res.json({
    ...
    description:
      '민감한 작업(발송처리, 취소승인 등)을 안전하게 테스트하기 위한 Mock 서버',
    endpoints: {
      'GET /health': '서버 상태 확인',
...
app.listen(PORT, () => {
  ...
  console.log(
    '  - POST /v1/pay-order/seller/product-orders/dispatch (발송처리)',
  );
  console.log(
    '  - POST /v1/pay-order/seller/product-orders/:id/cancel/approve (취소승인)',
  );
  console.log(
    '  - POST /v1/pay-order/seller/product-orders/:id/return/approve (반품승인)',
  );
  console.log(
    '  - POST /v1/pay-order/seller/product-orders/:id/exchange/approve (교환승인)',
  );
  console.log('');
  console.log(
    '⚠️  주의: 이는 Mock 서버입니다. 실제 네이버 API 처리는 하지 않습니다.',
  );
});
