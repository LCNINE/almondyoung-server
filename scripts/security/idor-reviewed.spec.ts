...
const IDOR_REVIEWED: Record<string, { verdict: Verdict; evidence: string; predicate: string; note?: string }> = {
  ...
  'analytics GET /statistics/behavior': {
    ...
    predicate: '@UseGuards(JwtAuthGuard, AdminRealmGuard)',
    note: 'GA4 행동 통계(구매 퍼널·일별 전환율·상품별·기기별) — 쿼리 파라미터가 날짜/limit 뿐이고 응답도 전사 집계(개별 사용자 식별자 없음)라 IDOR 대상 아님. 컨트롤러 클래스에 JwtAuthGuard + AdminRealmGuard(staff role 강제).',
  },
  ...
  'ugc-service DELETE /reviews/:id': {
    verdict: 'SAFE',
    evidence: 'apps/ugc-service/src/reviews/services/reviews.service.ts:661',
  ...
  'ugc-service GET /reviews/me': {
    verdict: 'SAFE',
    evidence: 'apps/ugc-service/src/reviews/services/reviews.service.ts:787',
    predicate:
  ...
  'ugc-service PATCH /reviews/:id': {
    verdict: 'SAFE',
    evidence: 'apps/ugc-service/src/reviews/services/reviews.service.ts:605',
  ...
  'ugc-service POST /reviews/:id/reactions': {
    verdict: 'SAFE',
    evidence: 'apps/ugc-service/src/reviews/services/reviews.service.ts:387',
