import { DEFAULT_PRODUCT_DESCRIPTION_PROMPT, IMAGE_DIRECTIVE_RULES, buildSystemPrompt } from './prompt';

describe('buildSystemPrompt', () => {
  it('어드민이 저장한 프롬프트를 앞에 둔다', () => {
    expect(buildSystemPrompt('커스텀 프롬프트').startsWith('커스텀 프롬프트')).toBe(true);
  });

  // 이 규칙이 빠지면 이미지가 한 장도 본문에 안 들어간다 — 편집 내용과 무관하게 항상 붙어야 한다.
  it('어드민이 이미지 규칙을 지운 프롬프트를 저장해도 규칙이 붙는다', () => {
    expect(buildSystemPrompt('이미지 얘기는 한 줄도 없는 프롬프트')).toContain(IMAGE_DIRECTIVE_RULES);
  });
